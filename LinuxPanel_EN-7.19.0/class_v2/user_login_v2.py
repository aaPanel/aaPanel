...
class userlogin:
    ...
    def request_temp(self,get):
        try:
            if len(get.get_items().keys()) > 2: return public.lang("Parameter ERROR!")
            if not hasattr(get,'tmp_token'): return public.lang("Parameter ERROR!")
            if len(get.tmp_token) != 48: return public.lang("Parameter ERROR!")
            if not re.match(r"^\w+$",get.tmp_token):return public.lang("Parameter ERROR!")
            skey = public.GetClientIp() + '_temp_login'
            if not public.get_error_num(skey,10): return public.lang("10 consecutive authentication failures are prohibited for 1 hour")
