...
do
    ...
    def GetTaskSpeed(self, get):
        ...
        if (type(find) == str):
            return public.return_message(-1, 0, 'Query error, {}', (find,))
        if not len(find):
            return public.return_message(-1, 0, 'NO_TASK_AT_LINEUP', ("-2",))
        isTask = '/tmp/panelTask.pl'
        public.writeFile(isTask, 'True')
        echoMsg = {}
        ...
        if find['type'] == 'download':
            try:
                ...
                if len(tmp) < 10:
                    return public.return_message(-1, 0, 'NO_TASK_AT_LINEUP', ("-3",))
                echoMsg['msg'] = json.loads(tmp)
                echoMsg['isDownload'] = True
            except:
                db.Sql().table('tasks').where("id=?", (find['id'],)).save('status', ('0',))
                return public.return_message(-1, 0, 'NO_TASK_AT_LINEUP', ("-4",))
        else:
