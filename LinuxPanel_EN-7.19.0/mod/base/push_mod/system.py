        ...
        try:
            for t in task_list:
                ...
                if not template["used"]:
                    continue
                print('打印任务 000 -{}'.format(t))
                print(_PushRunner(t, template, self)())
                print('打印_PushRunner 111 -{}'.format(_PushRunner(t, template, self)()))
                print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>")
        except:
            # import public
            import traceback
    ...
    def number_rule(self, number_rule: dict) -> bool:
        ...
        if "total" in number_rule and isinstance(number_rule["total"], int) and number_rule["total"] > 0:
            ...
            if record_total >= number_rule["total"]:
                self.result['stop_msg'] = "The maximum number of times the limit is exceeded:{}".format(number_rule["total"])
    ...
    def send_message(self, push_data: dict):
        ...
        for sender_id in self.task["sender"]:
            ...
            if conf is None:
                continue
            if not conf["used"]:
                self.result["send_data"][sender_id] = "The alarm channel {} is closed, skip sending".format(conf["data"].get("title"))
                continue
            ...
            if isinstance(res, str) and res.find("Traceback") != -1:
                self.result["send_data"][sender_id] = "An error occurred during the execution of the message transmission, and the transmission was not successful"
            if isinstance(res, str):
