...
class ServicesTask(BaseTask):
    ...
    def get_push_data(self, task_id: str, task_data: dict) -> Optional[dict]:
        ...
        s_list = [
            ">Service type: " + task_data["project"],
            ">Service state: 【" + task_data["project"] + "】service has been discontinued"]
        ...
        if task_data["count"] == 1:
            ...
            if not self.get_server_status(task_data["project"]):
                self.restart = False
                s_list[1] = ">Service state: 【" + task_data["project"] + "】service restart failed"
            else:
                self.restart = True
                s_list[1] = ">Service state: 【" + task_data["project"] + "】ervice restarted successfully"
...
class PanelSafePushTask(BaseTask):
    ...
    def get_push_data(self, task_id: str, task_data: dict) -> Optional[dict]:
        ...
        if t_add > 0 or t_del > 0:
            s_list.append(
                ">Change of logged-in user: <font color=#ff0000> Total:{}, Add:{}, Delete:{}</font>.".format(total, t_add, t_del))

        # 面板日志发生删除
        t_add, t_del, total = self.get_records_calc('panel_logs_safe', DB('logs'), 1)
        if t_del > 0:
            s_list.append(">If the panel log is deleted, the number of deleted logs is as follows:<font color=#ff0000>{} </font>".format(t_del))
    ...
    def get_push_data(self, task_id: str, task_data: dict) -> Optional[dict]:
        ...
        if int(task_data["count"]) == 1:
            ...
            if result["status"] is True:
                self.restart = True
                s_list[2] = ">Project state: Check that the project status is stopped, and it has been restarted successfully"
            else:
                self.restart = False
...
class ViewMsgFormat(object):
    _FORMAT = {
        ...
        "4": (
            lambda x: "<span>Triggered by {} consecutive failed logins within {} minutes, and tested again every {} seconds</span>".format(
        ...
        "5": (
            lambda x: "<span>A notification is sent once when the service is stopped, and it is detected again after {} seconds</span>".format(x["task_data"].get("interval"))
        ),
        "9": (
            lambda x: "<span>A notification is sent when the item is stopped, and the test is repeated after {} seconds, {} times per day</span>".format(
                x["task_data"].get("interval"),
                x.get("number_rule", {}).get("day_num", 0))
        ),
        "6": (
            lambda x: "<span>Alerts are sent when dangerous operations such as user changes, panel logs are deleted, and developers are enabled</span>"
        ),
        "10": (
