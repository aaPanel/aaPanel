...
class ssl_info:
    ...
    def dump_pkcs12_new(self, key_pem=None, cert_pem=None, ca_pem=None, friendly_name=""):
        try:
            ...
            cert = self.analysis_certificate((cert_pem + ca_pem).encode())
    ...
    def verify_format(self,type_string,pem_string):
        """
        验证证书和私钥格式是否正确
        :param key_pem: 私钥内容
        :param cert_pem: 证书内容
        ...
        """
        if pem_string.strip() == "":
            return False, "The key or certificate cannot be empty."
        check_start_private = ['-----BEGIN PRIVATE KEY-----','-----BEGIN RSA PRIVATE KEY-----']
        check_start_certificate = ['-----BEGIN CERTIFICATE-----']
        check_end_private = ['-----END PRIVATE KEY-----','-----END RSA PRIVATE KEY-----']
        check_end_certificate = ['-----END CERTIFICATE-----']
        if type_string=='key':
            error_message = "The key format is incorrect, please check if it is the correct PEM format private key."
            for item in check_start_private:
                if item in pem_string:
                    try:
                        if pem_string.split(item)[0].strip() != "":return False, error_message
                    except:pass

            for item in check_end_private:
                if item in pem_string:
                    try:
                        if pem_string.split(item)[1].strip() != "":return False, error_message
                    except:pass
        else:
            error_message = "The certificate format is incorrect, please check if it is the correct PEM format certificate."
            for item in check_start_certificate:
                if item in pem_string:
                    try:
                        if pem_string.split(item)[0].strip() != "":
                            public.print_log('pem_string.split(item)[0].strip():{}'.format(pem_string.split(item)[0].strip()))
                            return False, error_message
                    except:pass
            for item in check_end_certificate:
                if item in pem_string:
                    try:
                        if pem_string.split(item)[1].strip() != "":
                            public.print_log('pem_string.split(item)[1].strip():{}'.format(pem_string.split(item)[1].strip()))
                            return False, error_message
                    except:pass
