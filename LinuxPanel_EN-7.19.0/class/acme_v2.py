...
class acme_v2:
    ...
    def cloud_check_domain(self,domain):
        try:
            result = requests.post('{}/api/panel/checkDomain'.format(public.OfficialApiBase()),{"domain":domain,"ssl":1},s_type=self._request_type).json()
    ...
    def claer_auth_file(self,index):
        ...
        write_log(public.lang('|-Verify the dir：{}', acme_path))
...
"""
    ...
    def check_auth_status(self, url, desired_status=None):
        ...
        while True:
            if desired_status == ['valid', 'invalid']:
                write_log(public.lang('|-{} Query verification results..', str(number_of_checks + 1)))
            ...
            if authorization_status in desired_status:
                if authorization_status == "invalid":
                    write_log("|-" + public.lang("Verification failed"))
                    try:
        ...
        if desired_status == ['valid', 'invalid']:
            write_log(public.lang("|-Verification succeeded!"))
    ...
    def get_error(self, error):
        ...
        if error.find("Max checks allowed") >= 0:
            ...
            return public.lang("Issuance failed, the root domain name of domain name %s exceeds the maximum weekly issuance limit!" % re.findall(r"hours:\s+(.+?),", error))
            ...
            return public.lang('Issuing failed, the domain {} has exceeded the limit of weekly reissues!',str(re.findall("exact set of domains: (.+):", error)))
            ...
            return public.lang('Unsupported domain name {}, please check the domain name is correct!',str(re.findall("Cannot issue for \"(.+)\":", error)))
...
'''
    ...
    def delete_order(self, get):
        if 'index' not in get and not get.index:
            return{'status': False, 'msg': "缺少必填参数", 'finish_list': []}
        order_index = get.index.split(',')
        ...
        for i in order_index:
            try:
                order_hash = exclude_data.get("exclude_hash_let", {}).get(i)
                if order_hash:
                    from sslModel import certModel
                    local = True if 'local' not in get else get.local
                    cloud = True if 'cloud' not in get else get.cloud
                    try:
                        certModel.main().remove_cert(ssl_hash=order_hash, local=local, cloud=cloud)
                    except:
                        pass
                    del exclude_data["exclude_hash_let"][i]
                del data['orders'][i]
                complete += 1
                finish_list.append({"status": True, "name": "let's Encrypt"})
            except:
                finish_list.append({"status": False, "name": "let's Encrypt"})
                fail += 1
        public.writeFile(path, json.dumps(exclude_data))
        public.writeFile(self._conf_file_v2, json.dumps(data))
        if 'd' in get:
            return data
        return {'status': True, 'msg': "已成功删除{}个订单，失败{}个".format(str(complete), str(fail)), 'finish_list': finish_list}
            ...
            try:
                ...
                for j in ns.response.answer:
                    for i in j.items:
                        ...
                        if txt_value == value:
                            write_log(public.lang("|-Local authentication succeeded!"))
                            return True
            except:
                try:
                    import dns.resolver
                    ...
                    return False
            time.sleep(3)
        write_log(public.lang("|-Local authentication failed!"))
        return True
    ...
    def apply_cert_api(self, args):
        ...
        if not find:
            return public.return_msg_gettext(False, public.lang("Website lost, unable to continue applying for certificate"))
        ...
        if count:
            try:
                ...
                return public.return_msg_gettext(False, public.lang("There is an issue with the current project configuration file, please rebuild it"))
        else:
            if re.match(r"^\d+$", args.auth_to):
                ...
                if not os.path.exists(args.auth_to):
                    return public.return_msg_gettext(False, public.lang("Invalid site directory, please check if the specified site exists!"))
        try:
            # 检查认证环境
    ...
    def check_auth_env(self, args, check=None):
        ...
        if args.auth_type in ['http', 'tls']:
            try:
                ...
                if type(data) == list:
                    ...
                    for x in data:
                        if rp_conf and x['sitename'] in rp_conf:
                            ...
                            if x['type']: return public.return_msg_gettext(False, public.lang("Your site has 301 Redirect on，Please turn it off first!"))
                ...
                if type(data) == list:
                    ...
                    for x in data:
                        if rp_conf and x['sitename'] in rp_conf:
                            ...
                            if x['type']: return public.return_msg_gettext(False, public.lang("Sites with reverse proxy turned on cannot apply for SSL!"))
                ...
                if data['status']:
                    return public.return_msg_gettext(False, public.lang("The website has been redirected, please close it before applying!"))
                # 判断是否强制HTTPS
                if s.IsToHttps(args.siteName):
                    if os.path.exists(self._stop_rp_file):
                        ...
                        return public.return_msg_gettext(False, public.lang("After configuring Force HTTPS, you cannot use [File Verification] to apply for a certificate!"))
                public.serviceReload()
            except:
                return False
        else:
            if args.auth_to.find('Dns_com') != -1:
                if not os.path.exists('plugin/dns/dns_main.py'):
                    return public.return_msg_gettext(False, public.lang("Please go to the software store to install [cloud analysis], and complete the domain name NS binding."))
        return False

    # DNS手动验证
    def apply_dns_auth(self, args):
        if not hasattr(args, "index") or not args.index:
            return public.return_msg_gettext(False, public.lang("Incomplete parameter information, no index parameter [index]"))
    ...
    def renew_cert(self, index, cycle=None):
        ...
        try:
            ...
            if not order_index:
                write_log(public.lang("|-No SSL certificate found within 30 days!"))
                self.get_apis()
...
if __name__ == "__main__":
    ...
    p.add_argument('--domain', default=None,
                   help=public.lang("Please specify the domain name to apply for a certificate"),
                   dest="domains")
    p.add_argument('--type', default=None, help=public.lang("Please specify verification type"),
                   dest="auth_type")
    p.add_argument('--path', default=None, help=public.lang("Please specify the website document root"),
                   dest="path")
    ...
    p.add_argument('--dns_secret', default=None, help=public.lang("Please specify DNSAPI secret"),
                   dest="secret")
        ...
        try:
            if not args.index:
                ...
                if args.dnsapi == '0':
                    ...
                    write_log(public.lang("|-Retry the command") + ": ./acme_v2.py --index=\"{}\"",cert['index'])
                    write_log(public.get_msg_gettext(
    ...
    if not cert:
        exit()
    write_log("=" * 65)
    write_log(public.lang("|-Certificate obtained successfully!"))
    write_log("=" * 65)
    write_log(
