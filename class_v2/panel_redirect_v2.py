...
class panelRedirect:
    ...
    def __CheckRedirectStart(self,get,action=""):
        ...
        if (isError != True):
            return public.return_message(-1,0, 'An error was detected in the configuration file. Please solve it before proceeding')
        if action == "create":
            #检测名称是否重复
            ...
            if self.__CheckRedirect(get.sitename,get.redirectname,is_error_page):
                return public.return_message(-1,0, 'Specified redirect name already exists')
        #检测目标URL格式
        rep = r"http(s)?\:\/\/([a-zA-Z0-9][-a-zA-Z0-9]{0,62}\.)+([a-zA-Z0-9][a-zA-Z0-9]{0,62})+.?"
        if 'tourl' in get and not re.match(rep, get.tourl):
            return public.return_message(-1,0, 'Target URL format is wrong %s' + get.tourl)
        ...
        if 'errorpage' not in get:
            ...
            if get.domainorpath == "domain":
                if not json.loads(get.redirectdomain):
                    return public.return_message(-1,0, 'Please select redirected domain')
            else:
                if not get.redirectpath:
                    return public.return_message(-1,0, 'Please enter redirected path')
                #repte = "[\\?\\=\\[\\]\\)\\(\\*\\&\\^\\%\\$\\#\\@\\!\\~\\`{\\}\\>\\<\\,\',\"]+"
                # 检测路径格式
                if "/" not in get.redirectpath:
                    return public.return_message(-1,0, 'Path format is incorrect, the format is /xxx')
            ...
            if repeatdomain:
                return public.return_message(-1,0, 'Redirected domain already exists {}' , (repeatdomain,))
            #检测路径是否有存在配置文件
            repeatpath = self.__CheckRepeatPath(get)
            if repeatpath:
                return public.return_message(-1,0, 'Redirected domain already exists {}' , (repeatpath,))
            #检测目标URL是否可用
            ...
            if get.domainorpath == "domain":
                for d in json.loads(get.redirectdomain):
                    ...
                    if d == tu:
                        return public.return_message(-1,0,public.get_msg_gettext('Domain name {} is the same as the target domain name, please deselect it',(d,)))
            ...
            if get.domainorpath == "path":
                ...
                for d in domains:
                    ...
                    if tu == ad:
                        return public.return_message(0,0,'{}, the target URL is the same as the redirected path',(tu,))

        #404页面重定向检测项
        else:
            if 'tourl' not in get and 'topath' not in get:
                return public.return_message(-1,0, 'Please select where you need to redirect to')
            #网站首页访问检测
            if 'topath' in get and get.topath == "/":
                ...
                if not self.__firsturl:return public.return_message(-1,0, 'The website cannot be accessed, please check whether the website is working properly')
    ...
    def CreateRedirect(self,get):
        ...
        return public.return_message(0,0, 'Successfully created file!')
    ...
    def ModifyRedirect(self,get):
        ...
        if 'errorpage' in get and get.errorpage in [1,'1']:
            ...
            if web_type == 'nginx':
                ...
                return public.return_message(-1,0,'web server not installed or unknown web server')
        #非404页面重定向
        else:
            self.SetRedirect(get)
            self.SetRedirectNginx(get)
...
"""
    ...
    def del_redirect_multiple(self,get):
        ...
        public.serviceReload()
        return public.return_message(0,0,{'msg': public.get_msg_gettext('Delete redirects [{}] successfully',(','.join(del_successfully),)), 'error': del_failed,
    ...
    def GetRedirectFile(self,get):
        ...
        for i in conf:
            if redirectname == i["redirectname"] and sitename == i["sitename"] and i["type"] != 1:
                return public.return_message(-1,0, 'Redirection suspended')
        f = files.files()
