        ...
        if 'errorpage' not in get:
            ...
            if get.domainorpath == "domain":
                if not json.loads(get.redirectdomain):
                    return public.return_message(-1, 0, public.lang("Please select redirected domain"))
            else:
                if not get.redirectpath:
                    return public.return_message(-1, 0, public.lang("Please enter redirected path"))
                #repte = "[\\?\\=\\[\\]\\)\\(\\*\\&\\^\\%\\$\\#\\@\\!\\~\\`{\\}\\>\\<\\,\',\"]+"
                # 检测路径格式
                if "/" not in get.redirectpath:
                    return public.return_message(-1, 0, public.lang("Path format is incorrect, the format is /xxx"))
    ...
    def GetRedirectFile(self,get):
        ...
        for i in conf:
            if redirectname == i["redirectname"] and sitename == i["sitename"] and i["type"] != 1:
                return public.return_message(-1, 0, public.lang("Redirection suspended"))
        f = files.files()
