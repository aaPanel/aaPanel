...
class ftp:
    ...
    def AddUser(self,get):
        ...
        try:
            if not os.path.exists('/www/server/pure-ftpd/sbin/pure-ftpd'): 
                return_message=public.return_msg_gettext(False,'Please install the Pure-FTPd service in the software store first.')
            ...
            if get['ftp_username'].strip().find(' ') != -1: 
                return_message=public.returnMsg(False,'Username cannot contain spaces')
                del return_message['status']
                return public.return_message(-1,0, return_message['msg'])
            ...
            if public.M('ftps').where('name=?',(get.ftp_username.strip(),)).count(): 
                return public.return_message(-1,0,'User [{}] exists!'.format(get.ftp_username))
            username = get['ftp_username'].strip()
            if re.search("[\\/\\\\:\\*\\?\"\'\\<\\>\\|]+",username):
                return_message=public.return_msg_gettext(False,"Name cannot contain /\\:*?\"<>| symbol")
                del return_message['status']
                return public.return_message(-1,0, return_message['msg'])
            password = get['ftp_password'].strip()
            if len(password) < 6: 
                return_message=public.return_msg_gettext(False, 'Password must be at least [{}] characters',("6",))
                del return_message['status']
                return public.return_message(-1,0, return_message['msg'])
            ...
            return_message=public.return_msg_gettext(False,'Failed to add')
            del return_message['status']
            return public.return_message(-1,0, return_message['msg'])
    ...
    def DeleteUser(self,get):
        ...
        try:
            ...
            if public.M('ftps').where("id=? and name=?", (id,username, )).count()==0:
                return_message=public.return_msg_gettext(False, 'DEL_ERROR')
                del return_message['status']
                return public.return_message(-1,0, return_message['msg'])
            ...
            return_message=public.return_msg_gettext(False,'Failed to delete')
            del return_message['status']
            return public.return_message(-1,0, return_message['msg'])
    ...
    def SetUserPassword(self,get):
        ...
        try:
            ...
            return_message=public.return_msg_gettext(False,'Failed to modify')
            del return_message['status']
            return public.return_message(-1,0, return_message['msg'])
    ...
    def SetStatus(self,get):
        ...
        try:
            ...
            return_message=public.return_msg_gettext(False,'{} FTP user failed!',(msg,))
            del return_message['status']
            return public.return_message(-1,0, return_message['msg'])
    ...
    def setPort(self,get):
        ...
        try:
            port = get['port'].strip()
            if not port: 
                return public.return_message(-1,0, 'Please enter an integer for the port')
            if int(port) < 1 or int(port) > 65535: 
                return public.return_message(-1,0, 'Port range is incorrect!')
            check_used = public.check_port_stat(int(port),public.GetLocalIp())
            if check_used == 2:
                return public.return_message(-1,0, 'Port[{}] is used!'.format(str(port)))
            data = public.ExecShell('lsof -i:' + str(port))[0]
            if len(data) !=0:return public.return_message(-1,0, 'Port[{}] is used!'.format(str(port)))
    ...
    def set_user_home(self,get):
        ...
        try:
            ...
            if not auth_conf:
                return_message=public.returnMsg(False,'FTP account has not been set up')
                del return_message['status']
                return public.return_message(-1,0, return_message['msg'])
            ...
            if not macth_conf:
                return_message=public.returnMsg(False, 'FTP account has not been set up1')
                del return_message['status']
                return public.return_message(-1,0, return_message['msg'])
            if len(macth_conf) > 1:
                return_message=public.returnMsg(False, 'Matching multiple configurations, this operation has been stopped!')
                del return_message['status']
                return public.return_message(-1,0, return_message['msg'])
            ...
            return_message=public.returnMsg(False,'EDIT_ERROR')
            del return_message['status']
            return public.return_message(-1,0, return_message['msg'])
