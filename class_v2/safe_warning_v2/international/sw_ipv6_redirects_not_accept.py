...
_title = 'Ensure IPv6 redirects are not accepted'
_version = 2.0
_ps = 'Check if IPv6 redirects are rejected (only detected when IPv6 is enabled)'
_level = 2
_date = '2025-01-15'
...
_help = ''
_remind = 'This setting prevents the system from accepting ICMP redirects. ICMP redirects inform the system about alternate routes for sending traffic. It is recommended that systems not accept ICMP redirects as they may be spoofed and route traffic to compromised computers. Setting hard routes within the system (usually a single default route to a trusted router) protects the system from erroneous routes.'
...
def check_run():
    try:
        ...
        return False, 'IPv6 redirects not disabled in /etc/sysctl.conf: {}'.format(','.join(miss))
    except:
        return True, 'No risk'
