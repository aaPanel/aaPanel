...
from projectModelV2.watchModel import use_project_watch, add_project_watch, del_project_watch
...
class main(projectBase):
    ...
    def get_os_version(self, get):
        """
        @name Get the installation command for the operating system version
    ...
    def install_jdk(self, get):
        ...
        if jdk_version == '': return public.returnMsg(False, 'JDK version cannot be empty!')
        jdk_list = ['8', '11']
        if not jdk_version in jdk_list: return public.returnMsg(False, 'JDK version can only be 8 or 11!')
        ...
        if os_version == 'Centos':
            if jdk_version == '8':
                ...
                return public.returnMsg(False, 'JDK version can only be 8 or 11!')
            # Check if JDK is available
            jdk_data = self.get_jdk_version(None)
            if jdk_data['JDK%s' % jdk_version]['status']:
                # public.WriteFile(tmp_file,'Successify','a+')
                return public.returnMsg(True, 'Installation successful!')
            else:
                public.WriteFile(tmp_file, 'Error: Please check if the yum source is normal', 'a+')
            ...
            if jdk_data['JDK%s' % jdk_version]['status']:
                ...
                public.WriteFile(tmp_file, 'Error: Please check if the apt source is normal', 'a+')
                return public.returnMsg(False, 'Installation failed!')
        else:
            public.WriteFile(tmp_file, 'Error: Unsupported operating system', 'a+')
    ...
    def remove_jdk(self, get):
        ...
        @author lkq
        @param get
        @param get.jdk_version JDK version
        @return
        '''
        jdk_version = get.jdk_version.strip()
        os_version = self.get_os_version(None)
        ...
        if os_version == 'Centos':
            if jdk_version == '8':
                ...
                return public.returnMsg(False, 'JDK version can only be 8 or 11!')
            # public.WriteFile(tmp_file,'Successify','a+')
            return public.returnMsg(True, 'Uninstallation successful')
        elif os_version == 'Ubuntu':
            if jdk_version == '8':
    ...
    def check_jdk(self, jdk_path=None):
        '''
        @name  Check if JDK version is available
        @author lkq<2021-08-25>
        @param get<dict_obj>
        @return string
        '''
    ...
    def replace_jdk_version(self, get):
        '''
        @name Modify JDK version
        @author lkq<2021-08-27>
        @param get.tomcat_start  tomcat startup script path
        @param get.jdk_path  JDK path
        @return string
        '''
        ...
        if not jdk_data: return public.returnMsg(False, 'Please enter the correct JDK path, for example: /www/server/jdk1.8/bin/java')
        if jdk_data:
            if not os.path.exists(jdk_data): return public.returnMsg(False, 'JDK directory does not exist!')
        if not jdk_data: return public.returnMsg(False, 'The entered JDK is not available!')
        tomcat_start = get.tomcat_start.strip()
        if not os.path.exists(tomcat_start): return public.returnMsg(False, 'Tomcat startup script does not exist!')
        ...
        if isinstance(tomcat_start_file, str):
            ...
            if jdk_path_re:
                ...
                return public.returnMsg(True, 'Modification successful!')
        return public.returnMsg(False, 'Modification failed!')
    ...
    def get_tomcat_info(self, version):
        ...
        @name Get Tomcat version information
        @author lkq<2021-08-27>
        @param version  Tomcat version 7 8 9
        @return string
        '''
        tmp = {}
    ...
    def install_tomcat(self, get):
        ...
        @author lkq<2021-08-27>
        @param get<dict_obj>
        @param get.version Version to install|uninstall
        @param get.type    install == Install  uninstall == Uninstall
        @return string
        '''
        ...
        if not get.type in type_list:
            return public.returnMsg(False, 'Installation and uninstallation can only be install or uninstall!')
        if version == "7":
            if os_ver == 'Ubuntu':
                return public.returnMsg(False, 'Operating system not supported!')
        ...
        if get.type == 'install':
            ...
            if not tomcat_status['tomcat' + version]['status']:
                ...
                return public.returnMsg(False, 'Uninstallation failed!')
...
"""
        ...
        if res.find("Network connection error") != -1:
            return None
        else:
            return res.split("\n")[-2].strip()
    ...
    def install_tomcat_new(self, get):
        ...
        @param get.version Version to install|uninstall
        @param get.type    install == Install  uninstall == Uninstall
        @param get.jdk_path Use a different JDK version
        @return string
        '''
        try:
            jdk_path = ""
        ...
        if jdk_path:
            if jdk_path.endswith("/bin/java"):
                jdk_path = jdk_path[:-9]
            _, error = public.ExecShell("{}/bin/java -version".format(jdk_path))  # Information in standard error
            # public.print_log(_)
            # public.print_log(error)
            if not (error.find("version") != -1 and error.find("Runtime") != -1):
                return public.returnMsg(False, "The specified JDK is not available")
        ...
        if download_url is None:
            error = (
                '<br>Error: The connection to the Baota official website is abnormal. Please follow the following steps to troubleshoot the problem and try again:<br>Solution:'
    ...
    def change_tomcat_jdk(get):
        ...
        if not os.path.exists(jdk_path):
            return public.returnMsg(False, "The specified JDK path does not exist")
        if jdk_path.endswith("/bin/java"):
            jdk_path = jdk_path[:-9]
        ...
        if not os.path.isdir(tomcat_path):
            return public.returnMsg(False, 'Tomcat{} is not installed!'.format(version))
        
        deemon_sh_path = "{}/bin/daemon.sh".format(tomcat_path)
        if not os.path.isfile(deemon_sh_path):
            return public.returnMsg(False, 'Tomcat{} startup file is missing!'.format(version))
        
        deemon_sh_data = public.readFile(deemon_sh_path)
        if not isinstance(deemon_sh_data, str):
            return public.returnMsg(False, 'Failed to read Tomcat{} startup file!'.format(version))
        
        # deemon_sh
        rep_deemon_sh = re.compile(r"^JAVA_HOME=(?P<path>.*)\n", re.M)
        re_res_deemon_sh = rep_deemon_sh.search(deemon_sh_data)
        if not re_res_deemon_sh:
            return public.returnMsg(False, 'Failed to parse Tomcat{} startup file!'.format(version))
        ...
        if jsvc_make_path is None:
            return public.returnMsg(False, 'Jsvc{} file is missing!'.format(version))
...
'''.format(jsvc_make_path, jdk_path, tomcat_path + "/bin/jsvc")
    ...
    def change_duli_tomcat_jdk(self, project_name, jdk_path):
        ...
        if not project_info:
            return public.returnMsg(False, "Project does not exist")
        project_conf = project_info['project_config']
        if project_conf["java_type"] != "duli":
            return public.returnMsg(False, "Project type error")
        ...
        if not os.path.isdir(tomcat_path):
            return public.returnMsg(False, 'Tomcat is not installed!')
        
        deemon_sh_path = "{}/bin/daemon.sh".format(tomcat_path)
        if not os.path.isfile(deemon_sh_path):
            return public.returnMsg(False, 'Tomcat startup file is missing!')
        
        deemon_sh_data = public.readFile(deemon_sh_path)
        if not isinstance(deemon_sh_data, str):
            return public.returnMsg(False, 'Failed to read Tomcat startup file!')
        
        # deemon_sh
        rep_deemon_sh = re.compile(r"^JAVA_HOME=(?P<path>.*)\n", re.M)
        re_res_deemon_sh = rep_deemon_sh.search(deemon_sh_data)
        if not re_res_deemon_sh:
            return public.returnMsg(False, 'Failed to parse Tomcat startup file!')
        ...
        if jsvc_make_path is None:
            return public.returnMsg(False, 'Jsvc file is missing!')
...
'''.format(jsvc_make_path, jdk_path, tomcat_path + "/bin/jsvc")
    ...
    def set_tomcat_duli_path(self, get, get_project_find=''):
        '''
        @name Change independent project path
        @author lkq<2021-09-17>
        @param get.project_path<int>
        @param get.project_name Project name
        '''
        project_name = get.project_name.strip()
        project_info = get_project_find
        if not project_info: return public.returnMsg(False, 'Failed to change project path!')
        if project_info['project_config']['java_type'] == 'duli':
            ...
            if not self.Initialization2(version = '7', site = project_name): return public.returnMsg(
                False, "Tomcat configuration file error, please check the configuration file"
                )
            ...
            if ret:
                ...
                return public.returnMsg(True, 'Project path changed successfully!')
            else:
                return public.returnMsg(False, 'Failed to change project path!')
        if project_info['project_config']['java_type'] == 'neizhi':
            project_path = get.project_path.strip()
            if not os.path.exists(project_path): os.makedirs(project_path)
            if not self.Initialization(
                version = project_info['project_config']['tomcat_version']
                ): return public.returnMsg(False, "Tomcat configuration file error, please check the configuration file")
            ret = self.Set_Domain_path(project_name, project_path)
            if ret:
                project_info['project_config']['project_cwd'] = project_path
                pdata = {
    ...
    def set_tomcat_duli_port(self, get, get_project_find=''):
        '''
        @name Change independent project port
        @author lkq<2021-08-27>
        @param get.port<int>
        @param get.project_name Project name
        '''
        project_name = get.project_name.strip()
        project_info = get_project_find
        if not project_info: return public.returnMsg(False, 'Project does not exist!')
        if project_info['project_config']['java_type'] == 'duli':
            # Independent project
            port = str(get.port)
            if self.check_port_is_used(port): return public.returnMsg(False, 'Port is already in use!')
            if not self.Initialization2(version = '7', site = project_name): return public.returnMsg(
                False, "Configuration file error, please check the configuration file"
                )
            ...
            if ret:
                ...
                public.M('sites').where('name=?', (project_name,)).update(pdata)
                # self.set_config(get)
                return public.returnMsg(True, 'Change successful!')
            else:
                return public.returnMsg(False, 'Change failed')
        if project_info['project_config']['java_type'] == 'neizhi':
            port = str(get.port)
            if self.check_port_is_used(port): return public.returnMsg(False, 'Port is already in use!')
            if not self.Initialization(
                version = project_info['project_config']['tomcat_version']
                ): return public.returnMsg(False, "Configuration file error, please check the configuration file")
            ret = self.TomcatSetPort(port = port)
            if ret:
    ...
    def get_jdk_path(self, version, is_independent=False):
        '''
        @name Get JDK version path
        @author lkq<2021-08-27>
        @param version<string>
        @param is_independent<bool> Whether it is an independent version
        @return string
        '''
        if is_independent:
    ...
    def format(self, em, level=0):
        '''
        @name Format configuration file
        @author lkq<2021-08-27>
        @param em<string>
        @param level<int>
        '''
    ...
    def save_tomcat(self):
        '''
        @name Save Tomcat configuration file
        @author lkq<2021-08-27>
        '''
    ...
    def get_vhost(self, domain):
        '''
        @name Retrieve the specified virtual host
        @author lkq<2021-08-27>
        @param domain<string>
        @return string
        '''
    ...
    def add_vhost(self, path, domain):
        '''
        @name tomcat Fill in virtual host
        @author lkq<2021-08-27>
        @param path<string>
        @param domain<string>
        @return bool
    ...
    def start_tomcat(self, get):
        ...
        if s_type == 'start':
            ...
            return public.returnMsg(True, 'Start successful')
        elif s_type == 'stop':
            ...
            public.ExecShell(stop)
            public.ExecShell(start)
            return public.returnMsg(True, 'Reload successful')
        else:
            return public.returnMsg(False, 'Invalid type')
    
    def pendent_tomcat_start(self, get):
        '''
            Tomcat Launch for Independent Projects
            @author lkq 2021-08-27
            @param get.domain  Project Domain Name
            @param get.type  Startup type start stop  reload
        '''
        version = get.domain
        s_type = get.type
        # Check if the service is running
    ...
    def pendent_tomcat_info(self, get=None, domain=None):
        '''
        @name Independent project information
        @author lkq 2021-08-27
        '''
    ...
    def exists_nginx_ssl(self, project_name):
        '''
            @name Determine whether the project is configured with Nginx SSL configuration
    ...
    def set_nginx_config(self, project_find):
        '''
            ...
            @author hwliang<2021-08-09>
            @param project_find: dict<Project Information>
            @return bool
        '''
    ...
    def _replace_nginx_conf(config_file, mut_config: dict) -> bool:
        """Attempt to replace Nginx configuration"""
    ...
    def exists_apache_ssl(self, project_name):
        '''
            @name Determine whether the project is configured with Apache SSL configuration
            @author hwliang<2021-08-09>
            @param project_name: string<Project Name>
            @return bool
        '''
    ...
    def set_apache_config(self, project_find):
        ...
        # Check if the old configuration file has SSL configured
        ...
        # Generate configuration based on the list of ports
        for p in ports:
            ...
            if p == 443 and is_ssl:
                ...
                if not os.path.exists(ssl_key_file): continue  # Skip if the certificate file does not exist
                ssl_config = '''#SSL
    SSLEngine On
    ...
    #HTTP_TO_HTTPS_END'''
            ...
            # Generate vhost main body configuration
            ...
            # Add port to the main configuration file
            if not p in [80]:
                s.apacheAddPort(p)
        ...
        if not os.path.exists(rewrite_file): public.writeFile(
            rewrite_file, '# Please fill in the pseudo-static rules or custom Apache configuration here\n'
            )
    ...
    def get_nginx_ssl_config(self, project_name):
        '''
            @name Get Nginx SSL configuration for the project
            @author hwliang<2021-08-09>
            @param project_name: string<Project Name>
            @return string
        '''
    ...
    def clear_config(self, project_name):
        '''
            @name Clear project configuration
            @author hwliang<2021-08-09>
            @param project_name: string<Project Name>
            @return bool
        '''
    ...
    def tomcat_vhost_delete(self, version, domains, get):
        '''
        @name Remove the built-in Tomcat virtual host
        @author lkq
        @param version: string
        @param domain: string
        @return bool
        '''
        if not self.Initialization(version): return public.returnMsg(False, "Configuration file error, please check the configuration file")
        ...
        if self.del_vhost(domain, context_path):
            ...
            get.type = 'reload'
            self.start_tomcat(get)
            return public.returnMsg(True, 'Deletion successful')
        return public.returnMsg(False, 'Does not exist')
    ...
    def Initialization2(self, version, site):
        '''
        @name Initialize independent project
        @author lkq 2021-08-28
        @param site: string<Project Name>
        @param version: string<Version>
        '''
        # ! The initialization of the project related to Tomcat has been moved to Tomcat version management
    ...
    def replce_tomcat_port(self, get):
        ...
        if version.startswith('tomcat') or version.startswith('Tomcat'):
            version = version.replace('tomcat', '').replace('Tomcat', '')
        if self.IsOpen(port): return public.returnMsg(False, 'The port is already occupied!')
        if not self.Initialization(version): return public.returnMsg(False, "Configuration file error, please check the configuration file")
        ret = self.TomcatSetPort(port = port)
        if ret:
            # Reload Tomcat
            get.type = 'reload'
            self.start_tomcat(get)
            return public.returnMsg(True, 'Port change successful')
        return public.returnMsg(False, 'Port change failed')
    
    def replace_duli_port(self, get):
        '''
        @name Change duli port
        @param get.port: int
        @param get.version: string
    ...
    # Retrieve the specified virtual host
    ...
    def TomcatSetPath(self, port):
        '''
        @name Change Tomcat project path
        @author lkq
        @param port: string<Port number>
        '''
        for i in self.__CONNECTROR:
    ...
    def set_site_port(self, port, version, domain):
        ...
        '''
        if self.IsOpen(port): return public.returnMsg(False, 'The port is already occupied!')
        if not self.Initialization2(version, domain): return public.returnMsg(False, "Configuration file error, please check the configuration file")
        ret = self.TomcatSetPort(port = port)
        if ret:
            fs = firewalls.firewalls()
            get = mobj()
    ...
    def _get_jdk_status(self):
        ...
        try:
            ...
            data = {
                ...
                "msg": "Java is a general-purpose, object-oriented programming language",
                "shell": "java.sh",
                "task": "1",
    ...
    def get_local_jdk_version(self, get=None):
        ...
        if os.path.exists(self._panel_path + '/data/get_local_jdk.json'):
            ...
            if isinstance(ret2, str):
                ret2 = json.loads(ret2)
                for i in range(len(ret2)):
                    ret.append({'name': 'Custom JDK', 'path': ret2[i], 'operation': 1, 'is_current': False})
    ...
    def install_jdk_new(self, get):
        try:
            if not hasattr(get, 'version'):
                return public.returnMsg(False, 'Version number cannot be empty')
            ...
            if os.path.exists('/www/server/java/' + get['version']):
                return public.returnMsg(False, 'Version already exists')
            if get['version'] not in jdk_version:
                return public.returnMsg(False, 'Version number does not exist')
            ...
            if res[0].find('Java installation successful') == -1:
                return public.returnMsg(False, 'Failed to install Java')
            return public.returnMsg(True, 'Java installed successfully')
        except:
            public.ExecShell('rm -rf /www/server/java/{}.*'.format(get['version']))
            return public.returnMsg(False, 'Failed to install Java')
    ...
    def add_local_jdk(self, get):
        ...
        if jdk in local_jdk: return public.returnMsg(False, 'JDK path already exists')
        jdk_path = jdk.split('/')
        if jdk_path[-1] != 'java': return public.returnMsg(False, 'Please provide the complete path, for example: /www/server/jdk1.8/bin/java')
        if not os.path.exists(jdk): return public.returnMsg(False, 'JDK path does not exist')
        if not os.path.exists(self._panel_path + '/data/get_local_jdk.json'):
            ...
            if ret[0].find('Error occurred') != -1:
                return public.returnMsg(False, 'JDK is not available')
            ...
            if isinstance(data, str):
                ...
                if jdk in data: return public.returnMsg(False, 'Already exists')
                data.append(jdk)
    ...
    def project_remove_domain(self, get):
        ...
        if not project_find:
            return public.returnMsg(False, 'The specified project does not exist')
        last_domain = get.domain
        domain_arr = get.domain.split(':')
        if len(domain_arr) == 1:
            domain_arr.append(80)
        if domain_arr[0] == get.project_name: return public.returnMsg(False, 'Cannot delete the current project domain')
        ...
        if len(public.M('domain').where('pid=?', (project_find['id'],)).select()) == 1:
            return public.returnMsg(False, 'The project requires at least one domain name')
        domain_id = public.M('domain').where('name=? AND pid=?', (domain_arr[0], project_id)).getField('id')
        if not domain_id:
            return public.returnMsg(False, 'The specified domain does not exist')
        ...
        public.WriteLog(self._log_name, 'Deleted domain {} from project: {}'.format(get.domain, get.project_name))
        self.set_config(get.project_name)
        self.del_hosts(domain_arr[0])
        return public.returnMsg(True, 'Domain deleted successfully')
    ...
    def project_get_domain(self, get):
        ...
        project_id = public.M('sites').where('name=?', (get.project_name,)).getField('id')
        if not project_id:
            return public.return_data(False, 'Site query failed')
    ...
    def project_add_domain(self, get):
        ...
        for domain in domains:
            ...
            if domain_arr[0] is False:
                res_domains.append({"name": domain, "status": False, "msg": 'Invalid domain format'})
                continue
            if len(domain_arr) == 1:
                domain_arr.append("")
            if domain_arr[1] == "":
            ...
            if not public.M('domain').where('name=?', (domain_arr[0],)).count():
                ...
                res_domains.append({"name": domain_arr[0], "status": True, "msg": 'Successfully added'})
                flag = True
            else:
                public.WriteLog(self._log_name, 'Error adding domain, domain {} already exists'.format(domain))
                res_domains.append(
                    {"name": domain_arr[0], "status": False, "msg": 'Error adding, domain {} already exists'.format(domain)}
                    )
        if flag:
            public.M('sites').where('id=?', (project_id,)).save(
                'project_config', json.dumps(project_find['project_config'])
    ...
    def stop_project(self, get):
        ...
        if not project_info: return public.returnMsg(False, 'The specified project does not exist')
        
        project_find = self.get_project_find(get.project_name)
        if project_find['edate'] != "0000-00-00" and project_find['edate'] < datetime.datetime.today().strftime("%Y-%m-%d"):
            return public.return_error('The current project has expired, please reset the project expiration date')
        
        if project_info['project_config']['java_type'] == 'springboot':
            # pid_file = "{}/{}.pid".format(self._springboot_pid_path,get.project_name)
            pid_file = project_info['project_config']['pids']
            if not os.path.exists(pid_file): return public.returnMsg(False, 'The project is not running')
            try:
                pid = int(public.readFile(pid_file))
            except:
                return public.returnMsg(False, 'The project is not running')
            pids = self.get_project_pids(pid = pid)
            if not pids: return public.returnMsg(False, 'The project is not running')
            self.kill_pids(pids = pids)
            if os.path.exists(pid_file): os.remove(pid_file)
            self.stop_by_user(project_info["id"])
            return public.returnMsg(True, 'Stopped successfully')
        if project_info['project_config']['java_type'] == 'duli':
    ...
    def restart_project(self, get):
        ...
        if not res['status']: return res
        return public.returnMsg(True, 'Restarted successfully')
    ...
    def start_project(self, get):
        ...
        '''
        project_find = self.get_project_find(get.project_name)
        if not project_find: return public.returnMsg(False, 'The specified project does not exist')
    ...
    '''.format(
            ...
            time.sleep(1)
            if not os.path.exists(pid_file):
                return public.returnMsg(False, 'Failed to start, please try switching the startup user')
            
            # 获取PID
            try:
                pid = int(public.readFile(pid_file))
            except:
                return public.returnMsg(True, 'Failed to start')
            time.sleep(0.4)
            pids = self.get_project_pids(pid = pid)
            # if not pids:
            #    if os.path.exists(pid_file): os.remove(pid_file)
    ...
    def return_jdkcmd(self, get):
        ...
        jdK_path = get.jdK_path.strip()
        cmd = get.cmd.strip()
        if not os.path.exists(jdK_path): return public.returnMsg(False, 'JDK path does not exist')
        cmd2 = cmd.split()
        if 'debug' in get:
            debug = 1
        else:
    ...
    def send_cmd(self, get):
        ...
        if not hasattr(get, 'project_jdk'):
            return public.returnMsg(False, 'Missing parameter! project_jdk')
        project_jdk = get.project_jdk
        if not os.path.exists(project_jdk): return public.returnMsg(False, 'Project JDK does not exist')
        project_jar = get.project_jar.strip()
        if not os.path.exists(project_jar): return public.returnMsg(False, 'Project jar does not exist')
        jmx_cmd = ""
    ...
    def unbind_extranet(self, get):
        ...
        public.WriteLog(self._log_name, 'Java project {}, external network mapping closed'.format(project_name))
        return public.returnMsg(True, 'External network mapping closed successfully')
    ...
    def bind_extranet(self, get):
        ...
        if not public.is_apache_nginx(): return public.returnMsg(False, 'Please install Apache or Nginx first!')
        project_name = get.project_name.strip()
        project_find = self.get_project_find(project_name)
        if not project_find: return public.returnMsg(False, 'The specified project does not exist')
        if not project_find['project_config']['domains']: return public.returnMsg(
            False, 'Please add at least one domain in the 【Domain Management】 option'
            )
        ...
        public.WriteLog(self._log_name, 'Java project {}, external network mapping enabled'.format(project_name))
        return public.returnMsg(True, 'External network mapping enabled successfully')
    ...
    def create_project(self, get):
        ...
        try:
            ...
            return public.returnMsg(False, 'Project type parameter error')
        if project_type == 1:
            # 检查Nginx/Apache 配置文件是否OK
            isError = public.checkWebConfig()
            if isError != True:
                return public.returnMsg(
                    False, 'WEB server configuration file error ERROR:<br><font style="color:red;">' + isError.replace(
                        "\n", '<br>'
                        ) + '</font>'
                    )
    ...
    def create_internal_project(self, get):
        ...
        @return string
        '''
        if not public.is_apache_nginx(): return public.returnMsg(False, 'Apache or Nginx is not installed')
        if not 'domain' in get: return public.returnMsg(False, 'Please specify a domain!')
        if not 'tomcat_version' in get: return public.returnMsg(False, 'Please specify a Tomcat version!')
        if not 'project_path' in get: return public.returnMsg(False, 'Please specify a project path!')
        ...
        # 判断域名是合法性
        if not self.is_domain(domain): return public.returnMsg(False, "Please enter the correct domain name")
        # 兼容PHP 判断域名是否存在
        if public.M('domain').where('name=?', domain).count():
            return public.returnMsg(False, 'The specified domain already exists: {}'.format(domain))
        ...
        if tomcat_info['stype'] == 'uninstall': return public.returnMsg(False, 'The specified Tomcat does not exist!')
        ...
        if not self.Initialization(tomcat_version): return public.returnMsg(
            False, "Tomcat%s configuration file error or service not installed" % tomcat_version
            )
        # 获取当前tomcat 的port
        ...
        if self.add_vhost(path = project_path, domain = domain):
            ...
            public.WriteLog(self._log_name, 'Add Java project{}'.format(domain))
            # 重载服务
            # 检查配置文件是否OK 如果存在问题则回退配置文件
            get.version = tomcat_version
            get.type = 'reload'
            self.start_tomcat(get)
            return public.returnMsg(True, "Add Java project successfully")
        else:
            return public.returnMsg(False, "Add Java project failed")
    ...
    def create_independent_project(self, get):
        ...
        if not hasattr(get, "tomcat_version"):
            return public.returnMsg(False, 'Tomcat version not specified')
        tomcat_version = str(get.tomcat_version)
        domain = get.domain.strip()
        if not 'project_name' in get:
            get.project_name = get.domain
        ...
        if os.path.exists(self.__site_path + domain): return public.returnMsg(
            False, "The specified site already exists. To create a new one, please delete %s" % self.__site_path + domain
            )
        if not os.path.exists(project_path):
            os.makedirs(project_path)
            public.set_own(project_path, 'www')
        ...
        if tomcat_version == 'tomcat7' or tomcat_version == '7':
            if not os.path.exists(self.__tomcat7_path_bak + '/conf/server.xml'): return public.returnMsg(
                False, "Tomcat7 configuration file does not exist, please reinstall Tomcat7"
                )
            public.ExecShell(
        ...
        if tomcat_version == 'tomcat8' or tomcat_version == '8':
            if not os.path.exists(self.__tomcat8_path_bak + '/conf/server.xml'): return public.returnMsg(
                False, "Tomcat8 configuration file does not exist, please reinstall Tomcat8"
                )
            public.ExecShell(
                'cp -r %s/* %s && chown -R www:www %s' % (
        ...
        if tomcat_version == 'tomcat9' or tomcat_version == '9':
            if not os.path.exists(self.__tomcat9_path_bak + '/conf/server.xml'): return public.returnMsg(
                False, "Tomcat9 configuration file does not exist, please reinstall Tomcat9"
                )
            public.ExecShell(
                'cp -r %s/* %s && chown -R www:www %s' % (
    ...
    <Engine defaultHost="localhost" name="Catalina">
        ...
        <Valve className="org.apache.catalina.valves.AccessLogValve" directory="logs" pattern="%h %l %u %t &quot;%r&quot; %s %b" prefix="localhost_access_log" suffix=".txt" />
            ...
            if not self._change_daemom_sh(
                ...
                return public.returnMsg(False, "Configuration file does not exist, please reinstall Tomcat and try creating the site again")
        ...
        if not self.Initialization2(tomcat_version, domain): return public.returnMsg(
            False, "Configuration file error or service not installed"
            )
        # 先改他的端口
        ...
        if ret:
            ...
            public.WriteLog(self._log_name, 'Added Java project {}'.format(domain))
    ...
    def create_spring_boot_project(self, get):
        ...
        if get.run_user != 'root':
            ...
            if isinstance(ret, str):
                ...
                if not re.search(rec, ret):
                    return public.returnMsg(
                        False, 'Springboot user establishment failed, suspected to be intercepted by security software. Manually establish user operations as follows: useradd -s /sbin/nologin springboot'
                        )
            else:
                return public.returnMsg(
                    False, 'Springboot user establishment failed, suspected to be intercepted by security software. Manually establish user operations as follows: useradd -s /sbin/nologin springboot'
                    )
        if not 'project_jdk' in get: return public.returnMsg(False, "Please enter your JDK path")
        if not 'project_jar' in get: return public.returnMsg(False, "Please enter your jar path")
        if not 'project_name' in get and get.project_name.strip(): return public.returnMsg(False, "Please enter your project name")
        if not 'port' in get: return public.returnMsg(False, "Please enter your project port")
        if not 'run_user' in get: return public.returnMsg(False, "Please enter your project user")
        # if not 'auth' in get: return public.returnMsg(False, "请输入是否开机自启动")
        if not 'project_cmd' in get: return public.returnMsg(False, "Please enter your project start command")
        if not hasattr(get, 'project_ps'): return public.returnMsg(False, "Please enter your project description")
        ...
        if public.M('sites').where('name=? ', (get.project_name,)).count():
            return public.return_error('Specified project name already exists: {}'.format(get.project_name))
        project_path = os.path.dirname(get.project_jar)
        if not public.check_site_path(project_path):
            a, c = public.get_sys_path()
            return public.returnMsg(False, 'Please do not place project files in the following critical directories: <br>{}'.format("<br>".join(a + c)))
        
        if get.is_separation:
            if public.get_webserver() == 'apache':
                return public.returnMsg(False, "Front-end and back-end separation is not supported with Apache")
            get.is_separation = 1
        ##静态资源目录
        ...
        if get.is_separation:
            if 'host_url' not in get or not get.host_url.strip():
                return public.returnMsg(False, "Please enter the backend address you need")
            else:
                if get.host_url[-1] == '/':
        ...
        if 'domains' in get:
            ...
            if len(domains) >= 1:
                get.bind_extranet = 1
                # Check if Nginx/Apache configuration files are OK
                isError = public.checkWebConfig()
                if isError != True:
                    return public.returnMsg(
                        False, 'WEB server configuration file error ERROR:<br><font style="color:red;">' + isError.replace(
            ...
            get.bind_extranet = 0
        if public.M('sites').where('name=?', (get.project_name,)).count():
            return public.returnMsg(False, 'Specified project name already exists: {}'.format(get.project_name))
        project_name = get.project_name.strip()
        ...
        if self.check_port(get.port):
            return public.returnMsg(
                False, 'The specified port is already occupied by another application. Please modify your project configuration to use a different port: {}'.format(get.port)
                )
        # 判断JDK路径是否存在
        if not os.path.exists(get.project_jdk): return public.returnMsg(False, 'Please enter the correct JDK path')
        domains = []
        if get.bind_extranet == 1:
            domains = get.domains
        for domain in domains:
            ...
            if public.M('domain').where('name=?', domain_arr[0]).count():
                return public.returnMsg(False, 'Specified domain already exists: {}'.format(domain))
        # 获取jar包的路径
        project_jar = get.project_jar.strip()
        if not os.path.exists(project_jar): return public.returnMsg(False, 'Please enter the correct jar package path')
        # 获取jar的根目录
        ...
        public.WriteLog(self._log_name, 'Added Java Springboot project {}'.format(get.project_name))
    ...
    def kill_pids(self, get=None, pids=None):
        ...
        if not pids: return public.returnMsg(True, 'No process')
        ...
        for i in pids:
            try:
                ...
                pass
        return public.returnMsg(True, 'All processes have ended')
    ...
    def get_project_list(self, get):
        ...
        if isinstance(data["data"], str) and data["data"].startswith("error"):
            raise public.PanelError("Database query error：" + data["data"])
    ...
    def fix_project(self, get):
        ...
        <Engine defaultHost="localhost" name="Catalina">
            ...
            <Valve className="org.apache.catalina.valves.AccessLogValve" directory="logs" pattern="%h %l %u %t &quot;%r&quot; %s %b" prefix="localhost_access_log" suffix=".txt" />
                ...
                os.system('rm -rf %s' % self.__site_path + domain)
                return public.returnMsg(False, "The configuration file does not exist. Please reinstall Tomcat and try creating a new website")
            if not self.Initialization2(tomcat_version, domain):
                return public.returnMsg(False, "Configuration file error or service not installed")
            ...
            if ret:
                ...
                public.ExecShell('sh %s' % self.__site_path + domain + '/bin/daemon.sh start')
                # 建立反代项目
                public.WriteLog(self._log_name, 'Successfully repaired Java project {}'.format(domain))
                return public.returnMsg(True, "Repair successful")
        if project_config['java_type'] == 'neizhi':
            if not os.path.exists(
            ...
            if not self.Initialization(project_config['tomcat_version']): return public.returnMsg(
                False, "Tomcat%s configuration file error or service not installed" % project_config['tomcat_version']
                )
            if self.add_vhost(path = project_info['path'], domain = project_info['name']):
                ...
                if self.get_vhost(project_info['name']):
                    return public.returnMsg(True, 'Already repaired successfully, please do not submit again')
                return public.returnMsg(True, 'Repair failed')
        else:
            return public.returnMsg(False, 'The project is not a standalone Tomcat project or a built-in Tomcat project')
    ...
    def get_project_info(self, get):
        ...
        if not hasattr(get, 'project_name'):
            return public.returnMsg(False, 'Please provide the project name!')
    ...
    def get_project_log_old(self, get):
        ...
        if project_info['project_config']['java_type'] == 'springboot':
            ...
            if not os.path.exists(log_file): return public.returnMsg(False, 'The log file does not exist!')
    ...
    def remove_project(self, get):
        '''
            ...
            @return dict
        '''
        if not 'project_name' in get: return public.returnMsg(False, 'Please provide the project name!')
        project_find = self.get_project_find(get.project_name)
        if not project_find:
            return public.returnMsg(False, 'The specified project does not exist: {}'.format(get.project_name))
        if not project_find: return public.returnMsg(False, 'The specified project does not exist!')
        if not project_find['project_config']: return public.returnMsg(False, 'The specified project does not exist!')
        if not project_find['project_config']['java_type']: return public.returnMsg(False, 'The specified project does not exist!')
        if project_find['project_config']['java_type'] == 'duli':
            # 关闭独立项目
            get.domain = project_find['name']
            get.type = 'stop'
            ...
            public.WriteLog(self._log_name, 'Deleted Java project {}'.format(get.project_name))
            ...
            return public.returnMsg(True, 'Project deleted successfully')
            ...
            return public.returnMsg(False, 'Invalid project type')
    ...
    def modify_project(self, get):
        ...
        project_find = self.get_project_find(get.project_name)
        if not project_find:
            return public.return_error('The specified project does not exist: {}'.format(get.project_name))
    ...
    def modify_project_springboot(self, get, project_find):
        ...
        if hasattr(get, 'port'):
            if hasattr(get, 'port'):
                try:
                    ...
                    return public.returnMsg(False, "Invalid port parameter")
            if int(project_find['project_config']['port']) != int(get.port):
                if self.check_port_is_used(get.get('port/port'), True):
                    return public.returnMsg(
                        False, 'The specified port is already in use by another application. Please modify your project configuration to use a different port, Port: {}'.format(
                            get.port
                            )
                        )
        ...
        if hasattr(get, 'project_cmd'):
            ...
            return public.returnMsg(False, 'Missing project_cmd parameter')
        # Check if the jar file is in the cmd
        if project_find['project_config']['project_cmd'].find(get.project_jar.strip()) == -1:
            return public.returnMsg(False, 'The project jar file name is not in the project startup command, please check')
        ...
        public.WriteLog(self._log_name, 'Modified Java project {}'.format(get.project_name))
        return public.returnMsg(True, 'Project modified successfully')
    ...
    def modify_project_duli(self, get, project_find):
        ...
        if falg:
            ...
            self.stop_project(get)
            self.start_project(get)
        return public.returnMsg(True, 'Project modification completed')
    ...
    def modify_project_neizhi(self, get, project_find):
        ...
        if hasattr(get, 'port'):
            if int(project_find['project_config']['port']) != int(get.port):
                if self.check_port_is_used(get.get('port/port'), True):
                    return public.returnMsg(
                        False, 'The specified port is already occupied by another application. Please modify your project configuration to use a different port: {}'.format(
                            get.port
                            )
                        )
                project_find['project_config']['port'] = int(get.port)
        ...
        if hasattr(get, 'jdk_path'):
            ...
            if ret['jdk_path']:
                if ret['jdk_path'] != get.jdk_path.strip():
                    ...
                    flag = True
                    if not os.path.exists(ret['jdk_path']):
                        return public.returnMsg(False, 'The current JDK is unavailable')
                    # 验证JDK是否存在
                    ret2 = self.replace_jdk_version(get)
                    if not ret2['status']:
                        # 恢复到之前的JDK
        ...
        for project_find in project_list:
            ...
            if not project_state:
                ...
                if not result['status']:
                    error_count += 1
                    error_msg = 'Automatically start Java projects [' + project_name + '] failed!'
                    public.WriteLog(self._log_name, error_msg)
                else:
                    success_count += 1
                    success_msg = 'Automatically start Java projects [' + project_name + '] succeeded!'
                    public.WriteLog(self._log_name, success_msg)
        if (success_count + error_count) < 1: return False
        dene_msg = 'A total of {} Java projects need to be started, {} succeeded, {} failed'.format(
    ...
    def del_project_dump(self, get):
        ...
        project_find = self.get_project_find(get.project_name)
        if not project_find:
            return public.returnMsg(False, 'The project does not exist')
        if not project_find['project_config']['java_type'] == 'springboot': return public.returnMsg(
            False, 'Only SpringBoot projects are supported'
            )
        if not os.path.exists(self._springoot_dump + '/' + get.project_name + '.json'): return public.returnMsg(
            False, 'The path does not exist in the current project'
            )
        if not os.path.exists(get.dump_name.strip()): return public.returnMsg(False, 'The file does not exist')
        try:
            ret = json.loads(public.ReadFile(self._springoot_dump + '/' + get.project_name + '.json'))
    ...
    def heapdump_project(self, get):
        ...
        project_find = self.get_project_find(get.project_name)
        if not project_find:
            return public.returnMsg(False, 'The specified project does not exist 2: {}'.format(get.project_name))
        if not project_find['project_config']['java_type'] == 'springboot': return public.returnMsg(
            False, 'Only Springboot projects are supported'
            )
        ...
        if not self.get_project_run_state(project_name = get.project_name):
            return public.returnMsg(False, 'The specified project is not running: {}'.format(get.project_name))
        # 获取pid
        ...
        if 'jmap' in path:
            ...
            if type == 'Finfo':
                ...
                if os.path.exists(path):
                    ...
                    return public.returnMsg(True, 'Dump file generated successfully. Content: %s' % ret[0])
                else:
                    return public.returnMsg(False, 'Failed to generate dump file. Content: %s' % ret[0])
            elif type == 'Fdump':
                # 强制生成堆转储快照
                ...
                return public.returnMsg(False, 'The specified type does not exist: {}'.format(type))
            ret = public.ExecShell(cmd, user = project_find['project_config']['run_user'])
            return public.returnMsg(True, ret)
        return public.returnMsg(False, 'The current JDK does not have jmap')
    ...
    def jhat_project(self, get):
        ...
        if not os.path.exists(dump_path):
            return public.returnMsg(False, 'Dump file does not exist: {}'.format(dump_path))
    ...
    def jstack_project(self, get):
        ...
        if 'jstack' in path:
            ...
            ret = public.ExecShell(cmd, user = project_find['project_config']['run_user'])
            return public.returnMsg(True, ret)
        return public.returnMsg(False, 'The current JDK does not have jstack')
    ...
    def multi_remove_project(self, get):
        ...
        if not projects:
            return public.returnMsg(False, "No sites selected for deletion")
        ...
        for project in projects:
            ...
            if project['project_config']['java_type'] == 'duli':
                ...
                _error_type.append("Project:[{}], Invalid project type\n".format(project["name"]))
        if _error_type:
        ...
        public.WriteLog(self._log_name, f'Batch delete Java projects:[{"; ".join([i["name"] for i in projects])}]')
        # 获取域名列表 并删除host
        for project in projects:
        ...
        return {"status": True, "msg": "Delete successful", "project_names": pjnames}
    ...
    def _multi_remove_neizhi(self, projects):
        ...
        for version, group_projects in version_group.items():
            if not self.Initialization(version):
                res_msg.append(
                    "Project[{}]".format(",".join([i["name"] for i in group_projects])) + ":Configuration file error, please check the configuration file"
                )
                continue
    ...
    def multi_set_project(self, get):
        ...
        if set_type not in ["start", "stop"]:
            return public.returnMsg(False, "Invalid operation type")
        if isinstance(project_names, list):
            pjnames = [i.strip() for i in project_names]
        else:
            pjnames = []
        ...
        for project in projects:
            ...
            if project['project_config']['java_type'] == 'neizhi':
                if project['project_config']['tomcat_version'] in neizhi_projects:
                    ...
                    error_list.append(
                        {
                            "project_name": project["name"], "msg": "Startup failed, Tomcat{} is not installed".format(
        ...
        if error_list:
            return {"status": True, "msg": "Some projects failed to start", "error_list": error_list, "project_names": pjnames}
        return {"status": True, "msg": "Startup successful" if set_type == "start" else "Shutdown successful", "project_names": pjnames}
    
    # springboot 停止
    def __stop_springboot_project(self, project_info):
        pid_file = project_info['project_config']['pids']
        if not os.path.exists(pid_file): return public.returnMsg(False, 'Project not started')
        try:
            pid = int(public.readFile(pid_file))
        except:
            return False, 'Project not started'
        pids = self.get_project_pids(pid = pid)
        if not pids: return False, 'Project not started'
        self.kill_pids(pids = pids)
        if os.path.exists(pid_file): os.remove(pid_file)
        self.stop_by_user(project_info["id"])
...
'''.format(
        ...
        time.sleep(1)
        if not os.path.exists(pid_file):
            return False, 'Startup failed, please try switching the startup user'
    ...
    def multi_check_bind_extranet(self, get):
        ...
        for project in projects:
            ...
            if not project['project_config']['domains']:
                error_list.append({'project_name': project["name"], "msg": 'Please add at least one domain in the "Domain Management" section'})
                pjnames.remove(project["name"])
                continue
            ...
            public.WriteLog(self._log_name, 'Java project {}, enabled external network mapping'.format(project))
        ...
        if error_list:
            return {
                "status": True, "msg": "Some sites failed to enable external network mapping", "error_list": error_list, "project_names": pjnames
            }
        
        return {"status": True, "msg": "External network mapping enabled successfully", "project_names": pjnames}
    ...
    def change_log_path(self, get):
        ...
        if not new_log_path or new_log_path[0] != "/":
            return public.returnMsg(False, "Invalid path")
        ...
        if project_info['project_config']['java_type'] == 'springboot':
            ...
            res = self.start_project(get)
            public.WriteLog(self._log_name, 'Modified Java project {}'.format(get.project_name))
            return public.returnMsg(True, "Project log path modified successfully")
        if project_info['project_config']['java_type'] == 'duli':
            ...
            if not self._change_daemom_sh(
                ...
                return public.returnMsg(False, "Failed to modify project log path")
            pdata = {
                'name': project_info["name"],
                'project_config': json.dumps(project_info['project_config'])
            }
        ...
        if project_info['project_config']['java_type'] == 'neizhi':
            ...
            res = self.stop_project(get)
            res = self.start_project(get)
            public.WriteLog(self._log_name, 'Modified Java-Tomcat{} internal project path successfully'.format(version))
            return public.returnMsg(True, "Project log path modified successfully")
    ...
    def del_crontab(self, name):
        ...
        @return
        """
        cron_name = f'[Do not delete] Java project [{name}] log rotation'
    ...
    def change_cronta(self, name, log_conf):
        ...
        cronInfo = public.M('crontab').where('name=?', (f'[Do not delete] Java project [{name}] log rotation',)).find()
        if not cronInfo:
        ...
        if cronInfo['status'] == 0: return False, 'The current task is stopped, please start the task before modifying!'
        ...
        public.WriteLog('Scheduled Task', 'Modified scheduled task [' + cronInfo['name'] + '] successfully')
        return True, 'Modified successfully'
    
    def mamger_log_split(self, get):
        """Manage log rotation tasks
        @author baozi <202-02-27>
        @param:
            get  ( dict ):  Contains name, mode, hour, minute
        @return
        """
        name = get.name.strip()
        project = self.get_project_find(name)
        if not project:
            return public.returnMsg(False, "No such project, please try refreshing the page")
        try:
            _compress = False
    ...
    def set_log_split(self, get):
        ...
        cronInfo = public.M('crontab').where('name=?', (f'[Do not delete] Java project [{name}] log rotation',)).find()
        if not cronInfo:
            return public.returnMsg(False, "This project has not set up a log rotation task")
        
        status_msg = ['Disabled', 'Enabled']
        status = 1
        import crontab
        recrontabMode = crontab.crontab()
        ...
        public.WriteLog('Scheduled Task', 'Modified scheduled task [' + cronInfo['name'] + '] status to [' + status_msg[status] + ']')
        return public.returnMsg(True, 'Settings updated successfully')
    ...
    def get_log_split(self, get):
        ...
        if self._check_old(project_conf):
            return {
                "status": False,
                "msg": "After updating the version, you need to restart the project to enable the log rotation task. It is recommended to find a suitable time to restart the project.",
                "is_old": True
            }
        cronInfo = public.M('crontab').where('name=?', (f'[Do not delete] Java project [{name}] log rotation',)).find()
        if not cronInfo:
        ...
        if "log_conf" not in project_conf["project_config"]:
            return public.returnMsg(False, "Log rotation configuration is missing, please try resetting it")
    ...
    def _ckeck_add_domain(self, site_name, domains):
        ...
        if no_ssl:
            return {
                "domains": domains,
                "not_ssl": no_ssl,
                "tip": "This site has enabled SSL certificates, but the newly added domains: {} cannot match the current certificate. If needed, please reapply for the certificate.".format(
                    str(no_ssl)
                )
            }
        return {"domains": domains}
    ...
    def get_jmx_config(self, get):
        if not hasattr(get, "site_id"):
            return public.returnMsg(False, "Missing parameter! site_id")
        if not str(get.site_id).isdigit():
            return public.returnMsg(False, "Invalid parameter! site_id")
        project_config = public.M('sites').where('project_type=? AND id=?', ('Java', get.site_id)).getField(
            "project_config"
        )
        if not project_config:
            return public.returnMsg(False, "Website information does not exist!")
    ...
    def set_jmx_config(self, get):
        ...
        return public.returnMsg(True, "Configuration set successfully!")
    
    # Get jmx status information
    def get_status_info(self, get):
        ...
        if not project_config.get("jmx_status", False):
            return public.returnMsg(False, "JMX monitoring is not enabled")
        
        jmx_info = project_config.get("jmx_info")
        if jmx_info is None:
            return public.returnMsg(False, "Please set up JMX monitoring first")
        ...
        if not jmx_url:
            return public.returnMsg(False, "JMX monitoring is not set up")
        ...
        try:
            ...
            return public.returnMsg(False, "Connection failed! {}".format(jmx_url))
        ...
        for metric in metrics:
            ...
            if metric.attributeKey:
                ...
                if value == -1:
                    ...
                    value = "{} seconds".format(int(value) / 1e9)
    ...
    def get_project_status(self, project_id):
        # Only used in project stop alerts
    ...
    def set_jdk_environment(self, get):
        ...
        # 检查提供的JDK路径是否有效
        if not os.path.exists(jdk_path):
            return "The provided JDK path does not exist."
        ...
        if os.path.exists(profile_path):
            ...
            try:
                ...
                return public.returnMsg(False, "Setting failed, please check if you have enabled system hardening!")
        
        else:
            return public.returnMsg(False, "Setting failed!")
    ...
    def get_system_user_list(self, get):
        ...
        try:
            ...
            pass
        return public.return_message(0,0,list(user_set))
