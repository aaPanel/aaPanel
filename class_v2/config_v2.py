...
class config:
 ...
 # 取Session缓存方式
    ...
    def auto_cli_php_version(self,get):
        ...
        if not os.path.exists(php_bin_src): return public.return_message(-1,0,'PHP is not installed')
    ...
    def get_cli_php_version(self,get):
        ...
        if len(php_versions)==0:
            return public.return_message(-1,0,'Failed to get php version!')
        del(php_versions[0])
        for v in php_versions:
            if link_re.find(v['version']) != -1: return public.return_message(0,0,{"select":v,"versions":php_versions})
    ...
    def set_cli_php_version(self,get):
        ...
        if not os.path.exists(php_bin_src): return public.return_message(-1, 0,'Specified PHP version not installed')
    ...
    def set_php_session_path(self,get):
        ...
        try:
            ...
            return public.return_message(-1, 0, str(ex))
        if public.get_webserver() == 'openlitespeed':
            return public.return_message(-1,0, 'The current web server is openlitespeed. This function is not supported yet.')
    ...
    def get_nps_new(self, get):
        ...
        try:
            ...
            return public.return_message(-1,0,"Failed to obtain questionnaire")
    ...
    def write_nps_new(self, get):
        ...
        if not hasattr(get, 'questions'):
            return public.return_message(-1,0, "questions Parameter error")
        else:
            try:
                ...
                for _, i in content.items():
                    if len(i) > 512:
                        ...
                        return public.return_message(-1,0, "The submitted text is too long, please adjust and resubmit (MAX: 512)")
            except:
                return public.return_message(-1,0, "questions Parameter error")
        ...
        if res['success']:
            return public.return_message(0,0, "Submitted successfully")
