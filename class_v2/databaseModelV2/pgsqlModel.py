            ...
            if self.__DB_USER == 'postgres' and self.__DB_HOST == 'localhost':
                if not self.__DB_PASS:
                    ...
                    try:
                        ...
                        pass
            self.__DB_CONN = psycopg2.connect(user=self.__DB_USER, password=self.__DB_PASS, host=self.__DB_HOST,
                                              port=self.__DB_PORT)
...
class main(databaseBase, panelPgsql):
    ...
    def get_list(self, args):
        ...
        return public.return_message(0, 0, self.get_base_list(args, sql_type='pgsql'))
    ...
    def get_sql_obj_by_sid(self, sid=0, conn_config=None):
        ...
        if sid:
            ...
            db_obj = panelPgsql()
        return db_obj
        # return public.return_message(0, 0, db_obj)
    ...
    def get_sql_obj(self, db_name):
        ...
        if db_name:
            ...
            if db_find['sid']:
                ...
                return public.return_message(0, 0, self.get_sql_obj_by_sid(db_find['sid']))
    ...
    def ToBackup(self, args):
        ...
        if int(find['sid']):
            ...
            shell = '{} "host={} port={} user={} dbname={} password={}" > {}'.format(sql_dump, info['db_host'],
                                                                                     info['db_port'], info['db_user'],
                                                                                     find['name'], info['db_password'],
                                                                                     backupName)
        else:
            args_one = public.dict_obj()
            port = self.get_port(args_one)
            shell = '{} "host=127.0.0.1 port={} user={} dbname={} password={}" > {}'.format(sql_dump, port['data'],
                                                                                            find['username'],
                                                                                            find['name'],
                                                                                            find['password'],
                                                                                            backupName)
    ...
    def ToDataBase(self, find):
        ...
        isError = self.IsSqlError(result)
        if isError != None and isError['status'] == False and isError[
            'msg'] == 'The specified database already exists, please do not add it repeatedly.': return 1
    ...
    def SyncGetDatabases(self, get):
        ...
        for item in data:
            ...
            if not dbname in nameArr:
                if sql.table('databases').add('name,username,password,accept,ps,addtime,type,sid,db_type', (
    ...
    def check_cloud_database_status(self, conn_config):
        ...
        try:
            ...
            sql_obj = panelPgsql().set_host(conn_config['db_host'], conn_config['db_port'], conn_config['db_name'],
                                            conn_config['db_user'], conn_config['db_password'])
