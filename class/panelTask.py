  ...
  `addtime`			INTEGER
    ...
    def get_task_log(self, id, task_type, num=5):
        ...
        if(task_type == '1'):
            ...
            if not speed_total:
                data = {'name':public.lang('Download file {}',filename),'total':0,'used':0,'pre':0,'speed':0,'time':0}
            else:
                ...
                data = {'name': public.lang('Download file {}',filename),'total': total, 'used': used, 'pre': speed_total[1], 'speed': speed_total[2], 'time': u_time}
        else:
