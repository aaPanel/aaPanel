...
class files:
    ...
    def CreateFile(self,get):
        ...
        try:
            if not self.CheckFileName(get.path): return public.returnMsg(False,'FILE_NAME_SPECIAL_CHARACTRES');
    ...
    def CreateDir(self,get):
        if sys.version_info[0] == 2: get.path = get.path.encode('utf-8').strip();
        try:
            if not self.CheckFileName(get.path): return public.returnMsg(False,'DIR_NAME_SPECIAL_CHARACTRES');
            if os.path.exists(get.path):
...
''' % (get.path,get.shell)
    ...
    def GetTmpFile(self,get):
        ...
        src_path = get.path
        get.path = os.path.join(save_path,public.Md5(get.path) + '.tmp')
        if not os.path.exists(get.path): return public.returnMsg(False,'HAVE_NOT_DRAFT')
