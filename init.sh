...
panel_start()
{
        ...
        if [ "$isStart" == '' ];then
                ...
                if [ "$isStart" == '' ];then
                        ...
                        echo '------------------------------------------------------'
                        echo -e "\033[31mError: aaPanel service startup failed.\033[0m"
                fi
                echo -e "	\033[32mdone\033[0m"
        else
                echo "Starting aaPanel... aaPanel (pid $(echo $isStart)) already running"
        fi
        ...
        if [ "$isStart" == '' ];then
                ...
                if [ "$isStart" == '' ];then
                        ...
                        echo '------------------------------------------------------'
                        echo -e "\033[31mError: aaTask service startup failed.\033[0m"
                        return;
                fi
                echo -e "	\033[32mdone\033[0m"
        else
                echo "Starting aaTasks... aaTasks (pid $isStart) already running"
        fi
}

panel_stop()
{
	echo -e "Stopping aaTasks...\c";
...
panel_status()
{
        ...
        if [ "$isStart" != '' ];then
                echo -e "\033[32maaPanel (pid $(echo $isStart)) already running\033[0m"
        else
                echo -e "\033[31maaPanel not running\033[0m"
        fi
        
        isStart=$(ps aux |grep 'task.py'|grep -v grep|awk '{print $2}')
        if [ "$isStart" != '' ];then
                echo -e "\033[32maaTask (pid $isStart) already running\033[0m"
        else
                echo -e "\033[31maaTask not running\033[0m"
        fi
}
...
case "$1" in
        ...
        'default')
                ...
                echo -e "=================================================================="
                echo  "aaPanel-URL: http://$address:$port$auth_path"
