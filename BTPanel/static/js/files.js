...
function get_path_size(path) {
    var loadT = layer.msg(lan.files.calc_size, { icon: 16, time: 0, shade: [0.3, '#000'] });
...
function GetFiles(Path,sort) {
    ...
    if (searchV) {
        var loadT = layer.msg(lan.files.search_now, { icon: 16, time: 0, shade: [0.3, '#000']});
    }
	var totalSize = 0;
	$.post('/files?action=GetDir&tojs=GetFiles&p=' + p + '&showRow=' + showRow + search, data, function(rdata) {
        ...
        for (var i = 0; i < rdata.STORE.length; i++) {
            ...
            if (rdata.STORE[i].data.length > 0) {
                ...
                shtml += '<li><a style="padding-left: 15px;" href="javascript:;">No data</a></li>';
            }
            shtml += '</ul>';
        }
	...
	$.post('/files?action=GetFileAccess','filename='+encodeURIComponent(fileName),function(rdata){
		var layers = layer.open({
			...
			content:'<div class="setchmod bt-form ptb15 pb70">\
                        ...
                        <span><input type="checkbox" id="accept_all" checked /><label for="accept_all" style="position: absolute;margin-top: 4px; margin-left: 5px;font-weight: 400;">'+ lan.files.apply_to_subd +'</label></span>\
                        </div>\
						<div class="bt-form-submit-btn">\
							<button type="button" class="btn btn-danger btn-sm btn-title layer_close">'+lan.public.close+'</button>\
...
function RClick(type, path, name, file_store){
    ...
    options.items.push({
        text: "Favorites", onclick: function () {
            ...
            var file_data = {
                ...
                btns: [
                    bt.form.btn.close(),
                    {
                        title: 'Add', name: 'submit', css: 'btn-success', callback: function (rdata, load) {
                            var loading = bt.load();
...
function GetPathSize(){
	...
	layer.msg(lan.files.calc_now,{icon:16,time:0,shade: [0.3, '#000']})
...
function del_files_store(path, obj, parent_name) {
    ...
    if (parent_name != undefined) {
        ...
        msg = "Are you sure to delete the category [" + _item.name + "]?"
    }
    ...
    bt.send('get_files_store', 'files/get_files_store', {}, function (rRet) {
        ...
        if ($('#stroe_tab_list').length <= 0) {
            bt.open({
                ...
                shift: 5,
                shadeClose: false,
                content: "<div class='divtable pd15 style='padding-bottom: 0'><input name='type_name' class='bt-input-text mr5 type_name' placeholder='Please fill in the category name' type='text' style='width:420px'/><button id='btn_data_store_add' class='btn btn-success btn-sm' type='button' style='margin-bottom:10px'>Add</button><table width='100%' id='stroe_tab_list' class='table table-hover'></table><div class='page sitebackup_page'></div></div>",
                success: function () {
        ...
        function reload_sort_data(path) {
            var _tab = bt.render({
                ...
                columns: [
                    ...
                    {
                        field: 'opt', align: 'right', title: 'Operating', templet: function (item) {
                            var opt = ' <a class="btlink show_file_store_detail" >View</a>'
                            if (item.name != 'Default category') {
                                opt += ' | <a class="btlink del_file_store" onclick="del_files_store(\'' + path + '\',this)" >Del</a>';
                            }
                            return opt;
                        }
                    },
                ...
                store_type_index = bt.open({
                    ...
                    success: function () {
                        var _tab = bt.render({
                            ...
                            columns: [
                                { field: 'path', title: 'Path name' },
                                {
                                    field: 'opt', align: 'right', title: 'Operating', templet: function (item) {
                                        var opt = ' <a class="btlink del_file_store" onclick="del_files_store(\'' + path + '\',this,\'' + _item['name'] + '\')" >Del</a>'
                                        return opt;
                                    }
                                },
                            ],
