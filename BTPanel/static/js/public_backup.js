			...
			bt.open({
						...
						that.each(rdata, function (key, item) {
							...
							if (key === 0) {
								that.each(authNum, function (key1, numItem) {
									...
									' +
										numItem.num +
										(key1 === 0 ? '' : numItem.discount_rate != 1 ? '<em>' + (100 - numItem.discount_rate * 100) + '% off</em>' : '') +
										'\
									</li>';
				...
				renderAuthNums: function () {
					...
					that.get_product_discount_cache(config, function (rdata) {
						...
						for (var i = 0; i < rdata.length; i++) {
							if (rdata[i].cycle == selectedCycle) {
						...
						if (authNumData) {
							...
							for (var j = 0; j < authNumData.length; j++) {
								...
								var htmlNum =
									...
									'">' +
									authNumItem.num +
									(j === 0 ? '' : authNumItem.discount_rate !== 1 ? '<em>' + (100 - authNumItem.discount_rate * 100) + '% off</em>' : '') +
									'</li>';
