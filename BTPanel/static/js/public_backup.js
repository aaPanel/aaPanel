...
bt.crontab = {
	...
	edit_crontab:function(id,data){
		var that = this,loading = bt.load(lan.public_backup.submit_data);
			...
			var payhtml = '<div class="libPay" style="padding:15px 30px 30px 30px">\
					<div class="libPay-item f14 plr15 libPay-select">\
						...
						<div class="li-con c6">\
							...
							<p class="pro-info" style="position:absolute;top:151px;left:42px;color: #FF7301;font-size: 12px;display:none">'+lan.public_backup.pro_expire_must_renew_or_change_free_version+'</p>\
						</div>\
					</div>\
					<div class="libpay-con">\
					</div>\
...
bt.form ={
	...
	item:{
		data_access:{ title: lan.public_backup.access_right,items:[
		...
		password:{title: lan.public_backup.pass,name:'password',items:[
...
bt.data = {
	database:{
		root:{
			...
			area:'530px',
			list:[{title: lan.public_backup.rootpass,name:'password',items:[
		...
		data_add:{
			...
			area:'530px',
			list:[
				{title: lan.public_backup.db_name,items:[
					{name:'name',placeholder: lan.public_backup.new_db_name,type:'text',width:'65%',callback:function(obj){
				...
				{title: lan.public_backup.type,name:'dtype',type:'select',disabled:(bt.contains(bt.get_cookie('serverType'),'nginx') || bt.contains(bt.get_cookie('serverType'),'apache') ?true:false),items:[
		...
		data_access:{
			...
			btns:[
				bt.form.btn.close(),
				{title: lan.public_backup.submit,name:'submit',css:'btn-success',callback:function(rdata,load){
					var loading = bt.load();
		...
		data_pass : {
			...
			list:[
				...
				{title: lan.public_backup.pass,name:'password',items:[
			...
			btns:[
				...
				{title: lan.public_backup.submit,name:'submit',css:'btn-success',callback:function(rdata,load,callback){
					var loading = bt.load();
	...
	site:{
		add:{
			...
			area: '680px',
			list:[
				{title: lan.public_backup.domain,name:'webname',items:[
					{type:'textarea',width:'420px',callback:function(obj){
				...
				{title:lan.public_backup.root_dir,name:'path',items:[
				...
				{title:'FTP',items:[
					{name:'ftp',type:'select',items:[
						...
						if(obj.val()=='true'){
							...
							var item = {title:lan.public_backup.set_ftp,items:[
				...
				{title:lan.public_backup.db,items:[
					{name:'sql',type:'select',items:[
						...
						if(obj.val()!='false')
						{
							...
							var item = {title:lan.public_backup.db_set,items:[
				...
				{title:'Program type',type:'select',name:'type',disabled:(bt.contains( bt.get_cookie('serverType'),'IIS')?false:true),items:[
					{value:'PHP',title:'PHP'},
					{value:'Asp',title:'Asp'},
				...
				{title:lan.public_backup.php_v,name:'version',type:'select',items:[
					{value:'00',title:lan.public_backup.sitic}
                ]
                }, {
                    title: lan.public_backup.site_classification, name: 'type_id', type: 'select', width:'auto', items: [
	...
	ftp:{
		add:{
			...
			area:'530px',
			list:[
                { title: lan.public_backup.user_name, name: 'ftp_username', callback: function (obj) {
                    var defaultPath = $('#defaultPath').text();
				...
				{title:lan.public_backup.pass,name:'ftp_password',items:[
		...
		set_password:{
			...
			list:[
				...
				{title:lan.public_backup.pass,name:'new_password',items:[
					{type:'text',event: {css:'glyphicon-repeat',callback:function(obj){bt.refresh_pwd(16,obj);}}}
				]},
			],
			btns:[
