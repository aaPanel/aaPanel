	...
	var Config = (function (_super) {
			...
			Config.prototype.render = function () {
					...
					Promise.all([
							...
							var configInfo = resArr[0], twoStep = resArr[1], pawComplexity = resArr[2], bindUserInfo = resArr[3], messageChannelInfo = resArr[4], loginAlarmInfo = resArr[5], menuList = resArr[6];
			...
			Config.prototype.event = function () {
					...
					$('.savePanelConfig').click(function () { return __awaiter(_this, void 0, void 0, function () {
							var data, res, href;
							return __generator(this, function (_a) {
									switch (_a.label) {
											case 0:
					...
					$('.setPanelPort').click(function () { return _this.setPanelPortView(); });
									...
									return __generator(this, function (_a) {
											switch (_a.label) {
													case 0:
															...
															if (!vm.agreement)
																	return [2, this.$tips({ el: '#checkPanelPort', msg: 'Please tick the one I already know' })];
															...
															switch (_a.label) {
																	case 0:
																			if (row.type == 'ssh_login')
																					return [2, layer.msg('Do not support suspend SSH login alarm, if you want to stop it, please delete it directly.', { icon: 0 })];
											...
											template: function (row) {
													switch (row.type) {
															...
															case 'panel_pwd_endtime':
																	return "<span>Less than ".concat(row.cycle, " days remaining ").concat(typeof row.push_count != 'undefined' ? '(If not processed, it will be resent 1 time the next day for ' + row.push_count + ' days)' : '', "</span>");
															case 'ssh_login_error':
																	return "<span>Triggered by ".concat(row.count, " consecutive failed login attempts within ").concat(row.cycle, " minutes, to be detected again after every ").concat(row.interval, " seconds</span>");
															case 'panel_update':
									...
									switch (_a.label) {
											case 0:
													...
													switch (row.type) {
															...
															case 'panel_pwd_endtime':
																	if (row.type == 'ssl')
																			eData['project'] = row.site || 'all';
																	if (row.cycle == '' || row.cycle < 0)
																			return [2, this.$msg({ icon: 2, msg: 'Remaining days cannot be less than 1', time: 0, closeBtn: 2 })];
																	eData['cycle'] = Number(row.cycle);
																	eData['push_count'] = Number(row.push_count);
																	break;
															case 'ssh_login_error':
																	if (row.where1 == '' || row.where1 <= 0)
																			return [2, this.$msg({ icon: 2, msg: 'Trigger time cannot be less than 1', time: 0, closeBtn: 2 })];
																	if (row.count == '' || row.count <= 0)
																			return [2, this.$msg({ icon: 2, msg: 'Trigger times cannot be less than 1', time: 0, closeBtn: 2 })];
																	if (row.interval == '' || row.interval <= 0)
																			return [2, this.$msg({ icon: 2, msg: 'Interval cannot be less than 1', time: 0, closeBtn: 2 })];
			...
			Config.prototype.switchPushType = function (config, formData) {
					...
					var _checklist = [], isCheckType = [], siteList = [{ title: 'All Website', value: 'all' }], accountConfigStatus = false;
					if (!formData.type) {
							formData.type = 'ssl';
					...
					this.alertListModule.forEach(function (mod, i) {
							...
							_checklist.push({
									...
									title: (mod.name == 'wx_account' ? '<b style="color: #fc6d26;"> [Recommend]</b>' : '') +
											mod.title +
															...
															if (item.setup) {
																	if (item.name != 'sms') {
																			if (!$.isEmptyObject(item.data)) {
																					if (item.name == 'mail')
																							btnGroup += '<a class="btlink receiveMail">Recipient</a>&nbsp;|&nbsp;';
																					btnGroup +=
																							'<a class="btlink configEdit">' + lan.public.edit + '</a>&nbsp;|&nbsp;<a class="btlink alertTest">Test</a>&nbsp;|&nbsp;<a class="btlink uninstall_alert">' + lan.public.del + '</a>';
			...
			Config.prototype.renderAlertModelConfigInfo = function (mode) {
					var _info = '', noConfig = '<a class="bterror configEdit">Unconfigured</a>', _data = mode.data, isEmpty = $.isEmptyObject(_data);
					if (mode.setup) {
