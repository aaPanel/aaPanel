		...
		ReaderTable.prototype = {
				...
				bt.open({
					...
					content:
						...
						'<div class="batch_title"><span class><span class="batch_icon"></span><span class="batch_text">' +
							...
							' batch_tabel divtable" style="margin: 15px 30px 15px 30px;overflow: auto;max-height: 195px; border: 1px solid #ddd;"><table style="border: none;" class="table table-hover"><thead><tr><th>' +
							config.th +
							'</th><th style="text-align:right;width:170px;">' +
							lan['public'].result +
							'</th></tr></thead><tbody>' +
							config.html +
							'</tbody></table></div>',
					...
					if (typeof this.config.batch.config != 'undefined') {
						...
						if (this.checkbox_list.length > 0) {
							bt_select_btn
								...
								.text(lan['public'].please_choose + this.batch_active.title + '(' + lan['public'].selected + this.checkbox_list.length + ')');
						} else {
							bt_select_btn
								.addClass('bt-disabled btn-default')
								.removeClass('btn-success')
								.text(lan['public'].please_choose + this.batch_active.title);
						}
					} else {
						...
						if (this.checkbox_list.length > 0) {
							...
							bt_select_val
								.children()
								.eq(0)
								.html(lan['public'].please_choose + '<em></em>');
			...
			$reader_column_type: function (item, rows) {
				...
				switch (item.type) {
					...
					case 'switch': //开关类型
						...
						config = [
							'<div class="switch_group"><input class="btswitch btswitch-ios ' +
								className +
								'" id="' +
								_random +
								'" type="checkbox" ' +
							...
							if (!_hide) {
								if (items.template) {
									...
									if (_template == '') {
										...
										_html += item.group.length == index + 1 ? '' : '&nbsp;|&nbsp;';
									}
								} else {
									_html += item.group.length == index + 1 ? '' : '&nbsp;|&nbsp;';
								}
							...
							if (typeof active.tips === 'undefined' || active.tips) {
								...
								$.each(res.success, function (index, item) {
									html +=
										...
										'">' +
										item +
										'</span></td><td><div style="float:right;" class="size_ellipsis"><span style="color:#20a53a">' +
										lan['public'].success +
										'</span></div></td></tr>';
								});
								...
								if (!active.success) {
									...
									_that.$batch_success_table({
										title: 'Batch' + active.title + 'Success',
										th: active.theadName,
										html: html,
									});
									if (active.refresh) _that.$refresh_table_list(true);
						...
						start_batch: function (param, callback) {
							...
							if (active.load)
								this.loadT = layer.msg(
									lan['public'].executeing + active.title + '，<span class="batch_progress">' + lan['public'].schedule + ':0/' + this.check_list.length + '</span>,' + lan['public'].please_wait,
									{
										icon: 16,
										skin: 'batch_tips',
										shade: 0.3,
									...
									$('#layui-layer' + _this.loadT)
										...
										.html(
											'<i class="layui-layer-ico layui-layer-ico16"></i>' +
												lan['public'].executeing +
												active.title +
												'，<span class="batch_progress">' +
												lan['public'].schedule +
												':' +
												index +
												'/' +
												...
												'</span>,' +
												lan['public'].please_wait +
												(active.clear ? '<a href="javascript:;" class="btlink clear_batch" style="margin-left:20px;">cancel</a>' : '')
										);
								bt_tools.send(
									{
										url: this.config.url,
								...
								if (items.type == 'division') {
									...
									if (!items.group) {
										...
										template +=
											...
											_btn +
											' btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span style="margin-right:2px;">' +
											lan.site.category_manager +
											'</span><span class="caret" style="position: relative;top: -1px;"></span></button>\
						...
						case 'batch':
							...
							template =
								...
								(typeof item.config != 'undefined'
									? '<button type="button" class="btn btn-default btn-sm set_batch_option bt-disabled">Batch' + item.config.title + '</button>'
									: '<div class="bt_table_select_group bt-disabled not-select"><span class="bt_select_value"><span class="bt_select_tips">' +
									  lan['public'].select_opt_type +
									  '<em></em></span><span class="glyphicon glyphicon-triangle-bottom ml5"></span></span><ul class="bt_selects ">' +
									  _html +
									  '</ul></div><button class="btn btn-default btn-sm set_batch_option bt-disabled" >' +
									  item.buttonValue +
									  '</button>') +
								'</div>';
							break;
			...
			$reader_page: function (config, page) {
				...
				if (config.number && !page) {
					template =
						(config.page !== 1 ? '<a class="Pnum page_link_' + this.random + '"  data-page="1">Start</a>' : '') +
						(config.page !== 1 ? '<a class="Pnum page_link_' + this.random + '" data-page="' + (config.page - 1) + '">Prev</a>' : '') +
						(_that.data.length === config.number ? '<a class="Pnum page_link_' + this.random + '" data-page="' + (config.page + 1) + '">Next</a>' : '') +
						'<span class="Pcount">Page ' +
						config.page +
						'</span>';
					...
					if (config.jump) {
						...
						template +=
							'<div class="page_jump_group"><span class="page_jump_title">' +
							lan['public'].jump_to_page +
							'</span><input type="number" class="page_jump_input ' +
							inputName +
							'" value="' +
							config.page +
							'" /><span class="page_jump_title"></span><button type="button" class="page_jump_btn ' +
							btnName +
							'">' +
							lan['public'].confirm +
							'</button></div>';
			...
			$custom_page: function (total) {
				...
				return (
					page > 1 && config.page < page && (html += '<a class="Pstart" href="p=' + (config.page + 1) + '">Next</a><a class="Pstart" href="p=' + page + '">Last</a>'),
				...
				switch (item.type) {
					...
					case 'button':
						html +=
							...
							'<button class="btn ' +
							(item.hasOwnProperty('active') ? (item.active ? 'btn-success' : 'btn-default') : 'btn-success ') +
							' btn-' +
							(item.size || 'sm ') +
							' ' +
							eventName +
									...
									switch (items.type) {
										...
										case 'custom_select':
											...
											if (!select_value.hasClass('show')) {
												...
												// var $layer = $target.parents('.layui-layer');
												// var layerTop = $layer.length > 0 ? parseFloat($layer.css('top')) : 0;
												// var layerLeft = $layer.length > 0 ? parseFloat($layer.css('left')) : 0;
												// var offset = $target[0].getBoundingClientRect();
												// var top = offset.top - layerTop + offset.height + 2;
												// var left = offset.left - layerLeft;
												// $('.bt_select_list').css({
												// 	position: 'fixed',
		...
		var random = bt.get_random(5),
			layel = $(
				'<div class="layui-layer layui-layer-dialog layui-layer-msg layer-anim" id="' +
					random +
					'" type="dialog" style="z-index: 99891031;"><div class="layui-layer-content layui-layer-padding"><i class="layui-layer-ico layui-layer-ico16"></i>' +
					title +
					'，please wait...</div><span class="layui-layer-setwin"></span></div><div class="layui-layer-shade" id="layer-mask-' +
					random +
	...
	msg: function (param1, param2) {
		...
		if (typeof param1 === 'object') {
			if (typeof param1.status === 'boolean') {
				...
				if (!param1.status) config = $.extend(config, { time: !param2 ? 0 : 3000, closeBtn: 2, shade: 0.3 });
			}
		}
		...
		bt.open({
			...
			content:
				...
				'<div class="batch_title"><span class><span class="batch_icon"></span><span class="batch_text">' +
					...
					' batch_tabel divtable" style="margin: 15px 30px 15px 30px;overflow: auto;max-height: 195px; border: 1px solid #ddd;"><table style="border: none;" class="table table-hover"><thead><tr><th>' +
					config.th +
					'</th><th class="text-right" style="width:206px;">' +
					lan['public'].result +
					'</th></tr></thead><tbody>' +
					config.html +
					'</tbody></table></div>',
