var bt_file = {
	...
	file_header: { file_checkbox: 40, file_name: 'auto', file_tamper_core: 114, file_accept: 108, file_size: 90, file_mtime: 140, file_ps: 'auto', file_operation: 350, file_tr: 0, file_list_header: 0 },
	...
	event_bind: function () {
					...
					'<div class="file_tr createModel active">' +
						...
						(bt.get_cookie('rank') == 'icon'
							? '<span class="file_title file_' +
							  ...
							  (type == 'newBlankDir' ? 'New directory' : 'New blank file') +
							  '</textarea></span>'
							: '<span class="file_title file_' +
							  (type == 'newBlankDir' ? 'dir' : 'file') +
		...
		$('.tamper_core .nav_btn').on('click', function () {
			...
			bt.send('get_soft_find', 'plugin/get_soft_find', { sName: name }, function (rdata) {
				...
				// 	limit: 'pro',
				// 	closePro: true,
				// 	pluginName: 'Tamper-proof',
				// 	fun: function () {
				// 		product_recommend.recommend_product_view(rdata, { imgArea: ['1000px', '610px'] }, 'pro', 75, rdata.name, true);
				// 	},
				// });
			...
			function get_effective_path(path) {
				...
				bt.soft.get_soft_find('tamper_core', function (rdata_res) {
					...
					if (rdata_res.setup && rdata_res.endtime > -1) {
						console.log(1);
						bt_tools.send(
							{ url: '/tamper_core/get_effective_path.json', data: { path: path } },
							function (res) {
								if (!res.status) {
									if (res['tip']) {
										bt.confirm({ title: 'Tips', msg: res.tip + ', Is it open?' }, function (index) {
											layer.close(index);
											bt_tools.send(
										...
										if (res.msg.indexOf('get_effective_path') !== -1) {
											bt_tools.msg({ status: false, msg: 'Enterprise tampered revised version is too low, <a href="javascript:;"  class="btlink tamperCoreUpdate">Upgrade</a>！' });
											$('.tamperCoreUpdate')
												.unbind('click')
												.click(function () {
									...
									var url = '',
										exts_table_data = [];
									(is_lock = lock ? 'Turnning off' : 'Turning on'), (boxcontent = ''), (rules_path = ''), (suffix = '');
									if (pid !== 0) {
										//防篡改存在规则
									...
									if (data.type == 'dir') {
										...
										boxcontent = lock ? 'After turning off protection, all operations are allowed in this directory.' : 'After turning on protection, this directory denies all operations.';
									} else {
										...
										boxcontent = lock ? 'After turning off protection, all operations are allowed in this file.' : 'After turning on protection, this file denies all operations.';
									}
									var protection_arr = [],
										group = [],
										paramAction = [],
									...
									bt.open({
										type: 1,
										title: is_lock + ' protection [' + disposeData + ']',
										area: '500px',
										closeBtn: 2,
										btn: ['Confirm', 'Cancel'],
										shift: 5,
										...
										success: function (layero, index) {
											...
											$.each(action, function (index, item) {
												switch (item) {
													...
													case 'close':
														...
														break;
													case 'remove_wd':
														protection_arr.push({ title: 'Delete the white list directory', type: item, data: res.data.white_dir });
														paramAction.push({
															key: item,
														...
														break;
													case 'add_wd':
														protection_arr.push({ title: 'Delete the white list directory', type: item, data: path + '/' });
														paramAction.push({
															key: item,
															values: path + '/',
														});
													...
													case 'add_bf':
														if (res.data.white_files.indexOf(parentP + '/' + data.filename) === -1)
															protection_arr.push({
																title: 'Add protection of the file [' + parentP + '/' + disposeData + ']',
																type: item + '-2',
																data: parentP + '/' + disposeData,
																value: 1,
															});
														if (
															suffix !== '' &&
															...
															protection_arr.push({ title: 'Add all file suffixes to [' + suffix + '] protection', type: item + '-1', data: suffix });
														break;
													case 'remove_wf':
														protection_arr.push({
															title: 'Add protection of the file [' + parentP + '/' + data.filename + ']',
															type: item,
														...
														if (
															...
															protection_arr.push({ title: 'Add all file suffixes to [' + suffix + '] protection', type: 'add_bf-1', data: suffix });
														break;
													case 'remove_bf':
														if (suffix !== '') protection_arr.push({ title: 'Add all file suffixes to [' + suffix + '] protection', type: item, data: suffix });
														break;
													case 'add_wf':
														protection_arr.push({
															title: 'Disable protection of this file [' + parentP + '/' + data.filename + ']',
											...
											if (pid === 0 && data.type !== 'dir') {
												...
												if (suffix !== '') protection_arr.push({ title: 'Add protection of the suffix [' + suffix + ']', type: 'add_bf-1', data: suffix });
											}
										...
										yes: function (index, layero) {
											...
											if (pid !== 0 && data.type !== 'dir' && !settings.length) return layer.msg('Choose at least one');
		...
		$('.file_table_view .file_list_content').on('dblclick', '.file_tr', function (e) {
			var index = $(this).data('index'),
				data = that.file_list[index];
			if ($(e.target).hasClass('file_check') || $(e.target).parents('.foo_menu').length > 0 || $(e.target).hasClass('set_file_ps') || that.is_editor) return false;
			if (data.type == 'dir') {
	...
	set_file_view: function () {
		...
		var height = this.area[1] - $('.file_table_view')[0].offsetTop - 170;
		$('.file_bodys').height(this.area[1] - 52 - 36 - 24);
	...
	render_path_list: function (callback) {
		var that = this,
			html = '<div class="file_dir_omit hide" title="Expand hidden directories"><span></span><i class="iconfont icon-zhixiang-zuo"></i><div class="nav_down_list"></div></div>',
			path_before = '',
	...
	reader_file_list: function (data, callback) {
		...
		$('.file_table_view')
			...
			.addClass(model == 'list' ? 'list_view' : 'icon_view');
		$('.cut_view_model:nth-child(' + (model == 'list' ? '2' : '1') + ')')
			.addClass('active')
			.siblings()
			.removeClass('active');
		...
		this.get_dir_list(data, function (res) {
			...
			that.reader_file_list_content(that.file_list, function (rdata) {
				...
				$('.filePage').html(
					'<div class="page_num">Total ' +
						rdata.is_dir_num +
						' directory, ' +
						(that.file_list.length - rdata.is_dir_num) +
						'file(s), size:<a href="javascript:;" class="btlink" id="file_all_size">Click to calculate</a></div>' +
						page[0].outerHTML
				);
	...
	render_favorites_list: function () {
		...
		if (this.file_store_list.length > 0) {
			...
			html = '<li data-null style="width: 150px;"><i></i><span>(Empty)</span></li>';
		}
	...
	reader_file_list_content: function (data, callback) {
		...
		$.each(data, function (index, item) {
			...
			_html +=
				...
				item.soft_link +
				'</i></span>' +
				(item.caret ? '<span class="iconfont icon-favorites" style="' + (item.down_id != 0 ? 'right:30px' : '') + '" title="' + lan.files.file_has_been_favorite + '"></span>' : '') +
				...
				(item.is_lock ? 'green' : '') +
				'" title="Click' +
				(item.is_lock ? ' to close the anti -tampered, this file or directory can be modified' : ' to open the anti -tampered prevention, this file or directory will not be modified') +
				'" data-lock="' +
				item.is_lock +
				'" data-pid="' +
				...
				(item.is_lock ? 'Protected' : 'Unprotected') +
				'</span>' +
				...
				'</div>' +
				'</div>' +
				'<div class="file_td file_type hide"><span title="' +
				(item.type == 'dir' ? 'directory' : that.ext_type_tips(item.ext)) +
				'">' +
				(item.type == 'dir' ? 'directory' : that.ext_type_tips(item.ext)) +
				'</span></div>' +
				'<div class="file_td file_accept"><span>' +
				item.user +
				' / ' +
				item.root_level +
				'</span></div>' +
				'<div class="file_td file_size"><span>' +
				(item.type == 'dir' ? '<a class="btlink folder_size" href="javascript:;" data-path="' + path + '">Calculate</a>' : bt.format_size(item.size)) +
				'</span></div>' +
				'<div class="file_td file_mtime"><span>' +
				bt.format_data(item.mtime) +
				'</span></div>' +
				'<div class="file_td file_ps"><span class="file_ps_title" title="' +
				item.ps +
				'">' +
				(item.is_os_ps ? item.ps : '<input type="text" class="set_file_ps" data-value="' + item.ps + '" value="' + item.ps + '" />') +
				'</span></div>' +
				'<div class="file_td file_operation"><div class="set_operation_group ' +
				(that.is_mobile ? 'is_mobile' : '') +
				'">' +
	...
	render_file_disk_list: function () {
		...
		that.get_disk_list(function (res) {
			...
			$('.mount_disk_list').html(
				'<div class="disk_title_group_btn hide"><span class="disk_title_group">' +
					lan.files.mounted_disk +
					'</span><i class="iconfont icon-xiala"></i><ul class="nav_down_list">' +
					_li +
				...
				if (Array.isArray(item)) {
					...
					$.each(item[1], function (keys, items) {
						$children_list.append(
							$('<li><i class="file_menu_icon ' + keys + '_file_icon"></i><span>' + items + '</span></li>').on('click', { type: keys, data: config.data }, function (ev) {
	...
	batch_file_paste: function () {
		...
		this.check_exists_files_req({ dfile: this.file_path }, function (result) {
			if (result.length > 0) {
				...
				var mbody =
					'<div class="divtable" style="max-height:350px;overflow:auto;"><table class="table table-hover" width="100%" border="0" cellpadding="0" cellspacing="0"><thead><th>File name</th><th>Size</th><th>Last edit time</th></thead>\
                            <tbody>' +
					tbody +
					'</tbody>\
                            </table></div>';
	...
	replace_content_view: function () {
		layer.open({
			...
			closeBtn: 2,
			content:
				'<div class="replace_content_box" style="padding:20px 40px">' +
				...
				bt_file.file_path +
				'" type="text" style="width:570px" id="replaceContentPath">' +
	...
	searchReplaceContent: function () {
		...
		this.$http('files_search', data, function (res) {
			...
			$.each(res, function (fileName, item) {
				...
				match_file_html +=
					'<div class="match_content_item" data-file="' +
					fileName +
					'">' +
					'<div class="match_content_title">' +
					'<span class="match_result_file_title" title="' +
					fileName +
					'&nbsp;&nbsp;(Match ' +
					contentNum +
					' times)"><i class="glyphicon glyphicon-triangle-bottom"></i>' +
					fileName +
					'&nbsp;&nbsp;(Match ' +
					contentNum +
					' times)</span>' +
					'<a class="btlink pull-right editFile" data-filename="' +
					fileName +
					'">Edit</a>' +
					'</div>' +
				...
				$.each(item, function (index, lineItem) {
					...
					var html = $('<div></div>')
						.text(lineItem.trim())
						.html()
						.replace(reg, '<i style="font-weight: bold">' + $('<div></div>').text(data.text).html() + '</i>');
	...
	recycle_bin_view: function () {
		...
		layer.open({
			...
			content:
				...
				'</span>\
                    <button style="float: right" class="btn btn-default btn-sm" onclick="bt_file.CloseRecycleBin();">' +
				lan.files.recycle_bin_close +
				'</button>\
                    </div>\
                    <div class="re-con">\
                        <div class="re-con-menu">\
                            <p class="on" data-type="1">' +
			...
			switch (num) {
				case 1:
					for (var i = 0; i < rdata.dirs.length; i++) {
						...
						body +=
							'<tr>\
                          <td><span class=\'ico ico-folder\'></span><span class="tname" title="' +
							rdata.dirs[i].name +
							'">' +
							shortwebname +
							'</span></td>\
                          ...
                          <td>' +
							...
							'</td>\
                          <td style="text-align: right;">\
                              <a class="btlink" href="javascript:;" onclick="bt_file.ReRecycleBin(\'' +
							rdata.dirs[i].rname.replace(/'/, "\\'") +
							'\',this)">' +
							lan.files.recycle_bin_re +
							'</a>\
                              | <a class="btlink" href="javascript:;" onclick="bt_file.DelRecycleBin(\'' +
							rdata.dirs[i].rname.replace(/'/, "\\'") +
							'\',this)">' +
					...
					for (var i = 0; i < rdata.files.length; i++) {
						if (rdata.files[i].name.indexOf('BTDB_') != -1) {
							...
							body +=
								'<tr>\
                                    <td><span class="ico ico-' +
								that.get_ext_name(rdata.files[i].name) +
								'"></span><span class="tname" title="' +
								rdata.files[i].name +
								'">' +
				...
				{ type: 'checkbox', class: '', width: 18 },
				{
					...
					template: function (row) {
						return (
							...
							row.type +
							'"></i><span style="flex: 1; width: 0;" class="text-overflow">' +
							row.name +
							'</span></div>'
						);
					},
									...
									function (index) {
										...
										that.start_batch({}, function (list) {
											...
											for (var i = 0; i < list.length; i++) {
												var item = list[i];
												html +=
													'<tr><td><span class="limit-text-length" style="width: 170px;" title="' +
													item.name +
													'">' +
													item.name +
              ...
              <div class="property-box file_list_content  active">\
                ...
                <div class="attr-box">\
                  ...
                  <div class="attr-content" style="display: flex; align-items: center;">\
					...
					(res.path + '/' + data.filename).replace('//', '/') +
					'</span>\
										<i class="ico-copy cursor btcopy ml5 copyProperytPath" data-clipboard-text="' +
					(res.path + '/' + data.filename).replace('//', '/') +
					'" title="' +
				...
				success: function (layero, index) {
					...
					for (let i = 0; i < res.history.length; i++) {
						...
						html2 +=
							...
							item.st_mtime +
							'">view</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="javascript:;" class="btlink recovery_file_historys" data-history="' +
							item.st_mtime +
							'" data-path="' +
							data.path +
							'">restore</a></div></td></tr>';
	...
	open_video_play: function (data) {
		...
		layer.open({
			...
			content:
				...
				data.path +
				'" controls="controls" autoplay="autoplay" width="640" height="360">\
            Your browser does not support video tags\
                        </video></div><div class="video-list"></div>',
			success: function () {
				$.post('/files?action=get_videos', { path: path }, function (rdata) {
					var video_list =
							'<table class="table table-hover" style="margin-bottom:0;"><thead style="display: none;"><tr><th style="word-break: break-all;word-wrap:break-word;width:165px;">File name</th><th style="width:65px" style="text-align:right;">Size</th></tr></thead>',
						index = 0;
				...
				case 'copy':
					this.check_exists_files_req({ dfile: this.file_path, filename: _filename }, function (result) {
						if (result.length > 0) {
							...
							var mbody =
								'<div class="divtable"><table class="table table-hover" width="100%" border="0" cellpadding="0" cellspacing="0"><thead><th>File name</th><th>Size</th><th>Last edit time</th></thead>\
                                        <tbody>' +
								tbody +
								'</tbody>\
                                        </table></div>';
	...
	set_file_share: function (data) {
		...
		this.loadY = bt.open({
			...
			content:
				...
				'<div class="line"><span class="tname">Share name</span><div class="info-r"><input name="ps"  class="bt-input-text mr5" type="text" placeholder="No sharing name" style="width:270px" value="' +
				data.filename +
				'"></div></div>' +
	...
	file_share_view: function (datas, type) {
		...
		this.loadY = bt.open({
			...
			content:
				...
				'<div class="line"><span class="tname">Share name</span><div class="info-r"><input readonly class="bt-input-text mr5" type="text" style="width:365px" value="' +
				data.ps +
				'"></div></div>' +
				'<div class="line external_link"><span class="tname">Share chain</span><div class="info-r"><input readonly class="bt-input-text mr5" type="text" style="width:280px" value="' +
				download_url +
				...
				'"><span class="tname">Extraction code</span><div class="info-r"><input readonly class="bt-input-text mr5" type="text" style="width:243px" value="' +
				data.password +
				'"><button type="button" data-clipboard-text="Chain:' +
				download_url +
				' Extraction code:' +
				data.password +
				'"  class="btn btn-success copy_paw btn-sm btn-title">Copy link and code</button></div></div>' +
				'<div class="line"><span class="tname">Expiration date</span><div class="info-r"><span style="line-height:32px; display: block;font-size:14px">' +
				(data.expire > new Date('2099-01-01 00:00:00').getTime() / 1000 ? '<span calss="btlink">Permanent</span>' : bt.format_data(data.expire)) +
				'</span></div></div>' +
	...
	del_file_or_dir: function (data) {
		...
		if (that.is_recycle) {
			...
			bt.show_confirm(
				...
				'<i style="font-size: 15px;font-style: initial;color: red;">Recycle bin is not currently open, delete ' +
					(data.type == 'dir' ? 'directory' : 'file') +
					' cannot be restored after, continue?</i></span>',
				function () {
					that.del_file_req(data, function (res) {
						that.reader_file_list({ path: that.file_path });
						layer.msg(res.msg, { icon: res.status ? 1 : 2 });
	...
	set_file_authority: function (data, isPatch) {
		...
		that.get_file_authority({ path: data.path }, function (rdata) {
			var tex =
				'<div class="info-title-tips" style="padding-left: 7px;margin: 10px 23px;">\
			...
			that.loadY = layer.open({
				...
				content:
					'<div class="setchmod bt-form" style="padding: 0;">\
                            ...
                            <div class="chmodset" style="margin-bottom: 60px;">' +
					tex +
					'\
                                <fieldset>\
                                    <legend>' +
	...
	backup_files_permission: function (file, type) {
		...
		var layerss = layer.open({
			...
			btn: ['Yes', 'No'],
			content:
				'<div style="padding: 20px;margin: 0;font-size: 13px;">\
	...
	backup_list: function () {
		...
		that.$http('get_all_back', function (edata) {
			for (var i = 0; i < edata.length; i++) {
				...
				all_back +=
					'<tr style="padding: 5px;margin: 0;border: #e6e6e6 1px solid;border-top: none;" data-id="' +
					edata[i][0] +
					'">\
                    <td>' +
					date +
                    ...
                    <td style="max-width: 297px;min-width: 297px;">\
                        <a style="width: 287px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;display: inline-block;" class="btlink"  title="' +
					edata[i][3] +
					'" href="javascript:openPath(\'' +
					edata[i][3] +
					'\');">' +
					edata[i][3] +
					'</a>\
                    </td>\
                    <td style="max-width: 77px;min-width: 77px;overflow: hidden;text-overflow: ellipsis;text-align: left;" title="' +
					edata[i][1] +
					'">' +
					edata[i][1] +
					'</td>\
	...
	manage_backup: function () {
		...
		var layerss = layer.open({
			...
			shadeClose: false,
			content:
				'<div class="allback pd20" style="padding-bottom: 0;">\
		...
		that.$http('get_path_premissions', { path: fileName }, function (edata) {
			...
			if (edata.length == 0) {
				...
				for (var i = 0; i < edata.length; i++) {
					...
					tbody +=
						'<tr>\
                        <td><span style="display: inline-block;width: 60px; overflow: hidden;text-overflow: ellipsis;">' +
						edata[i][4] +
						'</span></td>\
                        <td>' +
						edata[i][2] +
						...
						date +
						'</td>\
                        <td style="color: #666;text-align: right;"><a data-time="' +
						edata[i][3] +
						'" class="btlink restore_backup">Restore</a> | <a data-id="' +
						edata[i][5] +
						'" class="btlink del_back">Del</a></td>\
                    </tr>';
				}
			}
			cont =
				'<div class="divtable" style="height: 260px; overflow: auto;padding:11px 13px 3px;">\
                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table table-hover">\
                    ...
                    <tbody class="backup_list" data-path="' +
				fileName +
				'">\
                        ' +
				tbody +
					...
					if (item.type == '1') {
						task_body +=
                            ...
                            <ul class="waiting-down-list">\
                                <li>\
                                    <div class="down-filse-name"><span class="fname" style="width:80%;" title="' +
							lan.layout.download +
							item.shell +
							'">' +
							lan.layout.download +
							...
							'/s</span><span style="margin-left: 20px;">' +
				...
				var loadT = layer.open({
					...
					success: function () {
						...
						$('.compress_file_view .line:nth-child(2)')
							.find('.info-r')
							.append('<span class="glyphicon glyphicon-folder-open cursor" style="margin-left: 10px;" onclick="ChangePath(\'compress_path\')"></span>');
					},
					yes: function () {
						var ress = form.getVal();
					...
					success: function () {
						...
						if (_type == 'zip') {
							...
							$('.unpack_file_view .line:nth-child(2)').append(
								'<div class="line"><span class="tname">Password</span><div class="info-r"><input type="text" name="z_password" class="bt-input-text " placeholder="No password let it empty" style="width:310px" value=""></div></div>'
							);
						}
					},
					yes: function () {
